...
/**
 * @brief The NonLinearProblemTestClass houses a very simple non-linear system
 * that is used to test the various non-linear solvers The linear solution is
 * obtained using LAPACK it uses a LAPACKMatrix and a Vector to store the
 * analytical jacobian and the right-hand side respectively
 *
...
public:
  ...
  virtual double get_residual_rescale_metric() const override { return 1.0; }
