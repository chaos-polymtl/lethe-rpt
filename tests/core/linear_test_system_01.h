...
/**
 * @brief The LinearProblemTestClass houses a very simple linear system that is
 * used to test the linear solver strategy. The linear solution is obtained
 * using LAPACK solve direct solver
 *
...
public:
  LinearProblemTestClass() : PhysicsSolver() {
  ...
  virtual void setup_preconditioner() override {}
  ...
  virtual void apply_constraints() override {
    throw std::runtime_error(
        "A linear system is being solved, no constraints to apply.");
  }

  virtual Vector<double> &get_evaluation_point() override {
    throw std::runtime_error(
        "A linear system is being solved, no evaluation point available.");
  };
  virtual Vector<double> &get_local_evaluation_point() override {
    throw std::runtime_error("A linear system is being solved, no local "
                             "evaluation point available.");
  };
  virtual Vector<double> &get_newton_update() override {
    throw std::runtime_error(
  ...
  virtual Vector<double> &get_present_solution() override {
    throw std::runtime_error(
        "A linear system is being solved, no present solution available.");
  };
  virtual Vector<double> &get_system_rhs() override { return system_rhs; };
  ...
  virtual double get_residual_rescale_metric() const override {
    throw std::runtime_error("A linear system is being solved, no residual "
                             "rescale metric available.");
  }
