...
SimulationControl::SimulationControl(const Parameters::SimulationControl &param)
    : method(param.method), assembly_method(param.method), current_time(0),
      time_step(param.dt), initial_time_step(param.dt),
      end_time(param.time_end), time_step_independent_of_end_time(
                                    param.time_step_independent_of_end_time),
      iteration_number(0), number_mesh_adapt(param.number_mesh_adaptation),
      CFL(0), max_CFL(param.maxCFL), residual(DBL_MAX),
      stop_tolerance(param.stop_tolerance),
      output_iteration_frequency(param.output_iteration_frequency),
      log_frequency(param.log_frequency), log_precision(param.log_precision),
      subdivision(param.subdivision), group_files(param.group_files),
      output_name(param.output_name), output_path(param.output_folder),
      output_boundaries(param.output_boundaries), first_assembly(true),
...
void SimulationControl::add_time_step(double p_timestep) {
...
bool SimulationControl::is_output_iteration() {
...
void SimulationControl::update_assembly_method() {
  if (bdf_start_method ==
    ...
    if (iteration_number <= 1 &&
      ...
      set_current_time_step(initial_time_step * (1 - startup_timestep_scaling));
...
unsigned int SimulationControl::get_number_of_stages(
  ...
  default:
    AssertThrow(
        ...
        ExcMessage("Unknown TimeSteppingMethod passed to "
                   "get_number_of_stages(). Please add the number of stages of "
                   "the new method in core/simulation_control.cc."));
    return 1;
  }
}

bool SimulationControl::is_verbose_iteration() {
  return (get_step_number() % log_frequency == 0);
}

void SimulationControl::save(const std::string &prefix) {
...
void SimulationControl::read(const std::string &prefix) {
  std::string filename = prefix + ".simulationcontrol";
...
std::vector<double> SimulationControl::get_checkpointed_simulation_control_info(
...
SimulationControlTransient::SimulationControlTransient(
    ...
    : SimulationControl(param), adapt(param.adapt),
      adaptative_time_step_scaling(param.adaptative_time_step_scaling),
      max_dt(param.max_dt), time_last_output(0.),
      output_time_frequency(param.output_time_frequency),
      output_times_vector(param.output_times_vector), output_times_counter(0),
...
void SimulationControlTransient::print_progression(
...
bool SimulationControlTransient::integrate() {
  if (!is_at_end()) {
...
bool SimulationControlTransient::is_output_iteration() {
...
void SimulationControlTransient::save(const std::string &prefix) {
...
void SimulationControlTransient::read(const std::string &prefix) {
...
void SimulationControlTransientDEM::print_progression(
    const ConditionalOStream &pcout) {
  if (!is_verbose_iteration())
    return;
...
bool SimulationControlSteady::integrate() {
  if (!is_at_end()) {
    previous_time = current_time;
    iteration_number++;
...
void SimulationControlSteady::print_progression(
    const ConditionalOStream &pcout) {
  if (!is_verbose_iteration())
    return;
...
bool SimulationControlSteady::is_at_end() {
...
void SimulationControlAdjointSteady::print_progression(
    const ConditionalOStream &pcout) {
  if (!is_verbose_iteration())
    return;
...
bool SimulationControlAdjointSteady::is_at_end() {
...
SimulationControlRayTracing::SimulationControlRayTracing(
    ...
    : SimulationControl(param), photon_handler(input_photon_handler) {}

bool SimulationControlRayTracing::integrate() {
  if (!is_at_end()) {
    iteration_number++;
    return true;
  }
...
bool SimulationControlRayTracing::is_at_end() {
  return photon_handler.n_global_particles() == 0;
}

void SimulationControlRayTracing::print_progression(
    const ConditionalOStream &pcout) {
