...
template <int dim>
void write_boundaries_vtu(const DataOutFaces<dim> &data_out_faces,
                          const std::string &folder, const double,
...
void write_boundaries_vtu_and_pvd(
    ...
    const std::string &file_prefix, const unsigned int digits) {
  const int my_id = Utilities::MPI::this_mpi_process(mpi_communicator);
...
template void write_vtu_and_pvd(
    PVDHandler &pvd_handler, const DataOutInterface<1, 2> &data_out,
    const std::string &folder, const std::string &file_prefix,
    const double time, const unsigned int iter, const unsigned int group_files,
    const MPI_Comm &mpi_communicator, const unsigned int digits);

template void write_vtu_and_pvd(
    PVDHandler &pvd_handler, const DataOutInterface<2, 2> &data_out,
    const std::string &folder, const std::string &file_prefix,
    const double time, const unsigned int iter, const unsigned int group_files,
    const MPI_Comm &mpi_communicator, const unsigned int digits);

template void write_vtu_and_pvd(
    PVDHandler &pvd_handler, const DataOutInterface<2, 3> &data_out,
    const std::string &folder, const std::string &file_prefix,
    const double time, const unsigned int iter, const unsigned int group_files,
    const MPI_Comm &mpi_communicator, const unsigned int digits);

template void write_vtu_and_pvd(
    PVDHandler &pvd_handler, const DataOutInterface<3, 3> &data_out,
    const std::string &folder, const std::string &file_prefix,
    const double time, const unsigned int iter, const unsigned int group_files,
    const MPI_Comm &mpi_communicator, const unsigned int digits);

template void write_vtu_and_pvd(
    PVDHandler &pvd_handler, const DataOutInterface<0, 2> &data_out,
    const std::string &folder, const std::string &file_prefix,
    const double time, const unsigned int iter, const unsigned int group_files,
    const MPI_Comm &mpi_communicator, const unsigned int digits);

template void write_vtu_and_pvd(
    PVDHandler &pvd_handler, const DataOutInterface<0, 3> &data_out,
    const std::string &folder, const std::string &file_prefix,
    const double time, const unsigned int iter, const unsigned int group_files,
    const MPI_Comm &mpi_communicator, const unsigned int digits);

template void write_boundaries_vtu(const DataOutFaces<2> &data_out_faces,
                                   const std::string &folder, const double time,
                                   const unsigned int iter,
                                   const MPI_Comm &mpi_communicator,
                                   const std::string &file_prefix,
                                   const unsigned int digits);

template void write_boundaries_vtu(const DataOutFaces<3> &data_out_faces,
                                   const std::string &folder, const double time,
                                   const unsigned int iter,
                                   const MPI_Comm &mpi_communicator,
                                   const std::string &file_prefix,
...
template void write_boundaries_vtu_and_pvd(
    PVDHandler &pvd_handler_boundary, const DataOutFaces<2> &data_out_faces,
    const std::string &folder, const double time, const unsigned int iter,
    const unsigned int group_files, const MPI_Comm &mpi_communicator,
    const std::string &file_prefix, const unsigned int digits);

template void write_boundaries_vtu_and_pvd(
    PVDHandler &pvd_handler_boundary, const DataOutFaces<3> &data_out_faces,
    const std::string &folder, const double time, const unsigned int iter,
    const unsigned int group_files, const MPI_Comm &mpi_communicator,
    const std::string &file_prefix, const unsigned int digits);
