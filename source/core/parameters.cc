...
DeclException2(PhaseChangeIntervalError, double, double,
               << "Liquidus temperature : " << arg1
               << " is not strictly superior to Solidus temperature: " << arg2
               << " The liquidus temperature specific is below or equal to the "
                  "solidus temperature."
               << " The phase change specific heat model requires that "
                  "T_liquidus>T_solidus.");

DeclException1(NumberOfFluidsError, int,
               << "Number of fluids: " << arg1
               << " is not 1 (single phase simulation) or 2 (VOF simulation). "
                  "This is currently not supported.");

DeclException1(NumberOfSolidsError, int,
               << "Number of solids: " << arg1
               << " is larger than 1. This is currently not supported.");

DeclException1(NumberOfMaterialInteractionsError, int,
               << "Number of material interactions: " << arg1
               << " is larger than 3. This is currently not supported.");

DeclException2(
    OrderOfFluidIDsError, int, int,
    << "The first fluid's id is " << arg1
    << " and the id of the second fluid is " << arg2
    << ". The first fluid's id should be lower than the second fluid's.");

DeclException1(
    TwoDimensionalLaserError, unsigned int,
    << "Laser beam orientation in : " << arg1
    << "-dimensional simulations cannot be defined in the z direction");

DeclException3(MultipleAdaptationSizeError, std::string, unsigned int,
               unsigned int,
               << "Error in 'mesh adaptation' : number of '" << arg1 << "' ("
               << arg2 << ") does not correspond to the number of 'variables' ("
               << arg3 << ")");

DeclException3(ParameterStrictlyGreaterThanError, std::string, double, double,
...
SizeOfSubsections get_size_of_subsections(const std::string &file_name) {
...
void SimulationControl::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("simulation control");
  {
    ...
    prm.declare_entry(
        "bdf startup method", "multiple step bdf",
    ...
    prm.declare_entry(
        "override time step on restart", "false", Patterns::Bool(),
        "Override checkpointed time-step upon restart <true|false>");
    prm.declare_entry("time step independent of end time", "true",
                      Patterns::Bool(),
                      "Ensures that the correct time step is kept when using "
                      "adaptive time step simulations");
    ...
    prm.declare_entry("max time step", "1e6", Patterns::Double(),
                      "Maximum time step value");
    prm.declare_entry("stop tolerance", "1e-10", Patterns::Double(),
                      "Tolerance at which the simulation is stopped");

    prm.declare_entry("adaptative time step scaling", "1.1", Patterns::Double(),
    ...
    prm.declare_entry("output time frequency", "-1", Patterns::Double(),
                      "Output time frequency");

    prm.declare_entry(
        "output boundaries", "false", Patterns::Bool(),
        "Output the boundaries of the domain along with their ID");

    prm.declare_entry("log frequency", "1", Patterns::Integer(),
                      "log frequency");

    prm.declare_entry("log precision", "6", Patterns::Integer(),
                      "Display precision when writing to log",
                      "This setting percolates to all output to the log");

    prm.declare_entry("output times", "-1", Patterns::List(Patterns::Double()),
                      "List of specific output times separated with a comma");

    prm.declare_entry("output control", "iteration",
                      Patterns::Selection("iteration|time"),
                      "The control for the output of the simulation results"
                      "Results can be either outputted at constant iteration "
                      "frequency or at constant time");
...
void SimulationControl::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("simulation control");
  {
    ...
    output_times_vector = convert_string_to_vector<double>(prm, "output times");
...
void Timer::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("timer");
  {
    ...
    prm.declare_entry(
        "write time in error table", "false", Patterns::Bool(),
...
void PowerLawParameters::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("power-law");
  {
    prm.declare_entry("K", "1.0", Patterns::Double(),
                      "Fluid consistency index");
    prm.declare_entry("n", "0.5", Patterns::Double(), "Flow behavior index");
    prm.declare_entry("shear rate min", "0.001", Patterns::Double(),
                      "Minimal shear rate magnitude");
  }
  prm.leave_subsection();
}

void PowerLawParameters::parse_parameters(ParameterHandler &prm,
...
void CarreauParameters::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("carreau");
  {
    prm.declare_entry("viscosity_0", "1.0", Patterns::Double(),
                      "Kinematic viscosity at rest");
    prm.declare_entry("viscosity_inf", "1.0", Patterns::Double(),
...
void CarreauParameters::parse_parameters(ParameterHandler &prm,
                                         const Dimensionality &dimensions) {
  prm.enter_subsection("carreau");
  {
...
void NonNewtonian::declare_parameters(ParameterHandler &prm) const {
...
void NonNewtonian::parse_parameters(ParameterHandler &prm,
                                    const Dimensionality &dimensions) {
  prm.enter_subsection("non newtonian");
  {
...
void ImmersedSolidTanhParameters::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("immersed solid tanh");
  {
    prm.declare_entry("tracer diffusivity inside", "1.0", Patterns::Double(),
                      "Tracer diffusivity inside the immersed solid");
    prm.declare_entry("tracer diffusivity outside", "1.0", Patterns::Double(),
                      "Tracer diffusivity outside the immersed solid");
    prm.declare_entry("tracer reaction constant inside", "0.0",
                      Patterns::Double(),
                      "Tracer reaction constant inside the immersed solid");
    prm.declare_entry("tracer reaction constant outside", "0.0",
                      Patterns::Double(),
                      "Tracer reaction constant outside the immersed solid");
    prm.declare_entry("thickness", "1.0", Patterns::Double(),
                      "Thickness to be used with the tanh function");
  }
  prm.leave_subsection();
}

void ImmersedSolidTanhParameters::parse_parameters(
    ParameterHandler &prm, const Dimensionality &dimensions) {
  prm.enter_subsection("immersed solid tanh");
  {
...
void ImmersedSolidGaussianParameters::declare_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("immersed solid gaussian");
  {
    prm.declare_entry("tracer diffusivity interface", "0.0", Patterns::Double(),
                      "Tracer diffusivity at the immersed solid interface");
    prm.declare_entry("tracer diffusivity bulk", "1.0", Patterns::Double(),
                      "Tracer diffusivity in the phase bulk");
    prm.declare_entry(
        "tracer reaction constant interface", "0.0", Patterns::Double(),
        "Tracer reaction constant at the immersed solid interface");
    prm.declare_entry("tracer reaction constant bulk", "0.0",
                      Patterns::Double(),
...
void ImmersedSolidGaussianParameters::parse_parameters(
    ParameterHandler &prm, const Dimensionality &dimensions) {
  prm.enter_subsection("immersed solid gaussian");
  {
...
void IsothermalIdealGasDensityParameters::declare_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("isothermal_ideal_gas");
  {
    prm.declare_entry("density_ref",
                      "1.2", // dry air's density at normal temperature and
                             // pressure (20 °C and 1 atm)
                      Patterns::Double(),
                      "Reference density of the gas in SI units for isothermal "
                      "ideal gas equation of state in density calculation");

    prm.declare_entry("R",
                      "287.05", // dry air's specific gas constant as default
                      Patterns::Double(),
                      "Specific gas constant in SI units for isothermal ideal "
                      "gas equation of state in density calculation");

    prm.declare_entry(
        "T",
        "293.15", // normal temperature (20°C) as a default
        Patterns::Double(),
        "Absolute temperature of the gas in kelvin (K) for isothermal ideal "
        "gas equation of state in density calculation");
  }
  prm.leave_subsection();
}

void IsothermalIdealGasDensityParameters::parse_parameters(
    ParameterHandler &prm, const Dimensionality &dimensions) {
  prm.enter_subsection("isothermal_ideal_gas");
  {
...
void SurfaceTensionParameters::declare_parameters(
    dealii::ParameterHandler &prm) {
  prm.declare_entry("surface tension coefficient", "0.0", Patterns::Double(),
                    "Surface tension coefficient for the corresponding pair of "
                    "fluids or fluid-solid pair");
  prm.declare_entry("reference state temperature", "0.0", Patterns::Double(),
                    "Temperature of the reference state corresponding to the "
                    "surface tension coefficient");
  prm.declare_entry("temperature-driven surface tension gradient", "0.0",
                    Patterns::Double(),
                    "Surface tension gradient with respect to the temperature "
                    "for the corresponding pair of fluids or fluid-solid pair");
  prm.declare_entry("solidus temperature", "0", Patterns::Double(),
                    "Temperature of the solidus for the corresponding pair of "
                    "fluids or fluid-solid pair");
  prm.declare_entry("liquidus temperature", "1", Patterns::Double(),
                    "Temperature of the liquidus for the corresponding pair of "
                    "fluids or fluid-solid pair");
}

void SurfaceTensionParameters::parse_parameters(
    const ParameterHandler &prm, const Parameters::Dimensionality &dimensions) {
...
void MobilityCahnHilliardParameters::declare_parameters(
    dealii::ParameterHandler &prm) {
  prm.declare_entry(
      "cahn hilliard mobility constant", "1e-7", Patterns::Double(),
      "Cahn-Hilliard mobility constant for the corresponding pair of fluids");
}

void MobilityCahnHilliardParameters::parse_parameters(
    const ParameterHandler &prm, const Parameters::Dimensionality &dimensions) {
  mobility_cahn_hilliard_constant =
      prm.get_double("cahn hilliard mobility constant");
  mobility_cahn_hilliard_constant *= dimensions.cahn_hilliard_mobility_scaling;
}

template <int dim>
void ConstrainSolidDomain<dim>::declare_parameters(
    dealii::ParameterHandler &prm,
    const unsigned int max_number_of_constraints) {
  prm.enter_subsection("constrain stasis");
  {
    prm.declare_entry(
        "enable", "false", Patterns::Bool(),
        "Enable/disable (true/false) the solid domain constraining feature.");

    prm.declare_entry("enable domain restriction with plane", "false",
                      Patterns::Bool(),
                      "Enable/disable (true/false) the definition of a plane "
                      "for geometrical\n"
                      " restrictions on the domain where the solid domain "
                      "constraining feature\n"
                      " is applied.");
    std::string default_entry_sting = (dim == 2) ? "0., 0." : "0., 0., 0.";
    prm.declare_entry("restriction plane point", default_entry_sting,
                      Patterns::List(Patterns::Double()),
                      "Domain restriction plane point coordinates.");
    prm.declare_entry(
        "restriction plane normal vector", default_entry_sting,
        Patterns::List(Patterns::Double()),
        "Domain restriction plane outward pointing normal vector.");

    prm.declare_entry("number of constraints", "0", Patterns::Integer(),
...
template <int dim>
void ConstrainSolidDomain<dim>::declare_default_entries(
    dealii::ParameterHandler &prm) {
  prm.declare_entry("fluid id", "0", Patterns::Integer(),
                    "Identifier of the fluid material that is constrained.");
  prm.declare_entry("phase fraction tolerance", "1e-4", Patterns::Double(),
  ...
  prm.declare_entry("min temperature", "-999", Patterns::Double(),
                    "Minimum temperature value of the fluid for it be "
                    "considered as a solid.");
  prm.declare_entry("max temperature", "0", Patterns::Double(),
                    "Maximum temperature value of the fluid for it be "
                    "considered as a solid.");
}

template <int dim>
void ConstrainSolidDomain<dim>::parse_parameters(
    dealii::ParameterHandler &prm) {
  prm.enter_subsection("constrain stasis");
  {
...
template <int dim>
void ConstrainSolidDomain<dim>::parse_constraint_parameters(
    const dealii::ParameterHandler &prm, const unsigned int constraint_id) {
...
void Stabilization::declare_parameters(ParameterHandler &prm) {
  ...
  {
    prm.declare_entry(
        "use default stabilization", "true", Patterns::Bool(),
        "Use the default stabilization method provided by the solver");
    prm.declare_entry(
        "stabilization", "pspg_supg",
    ...
    prm.declare_entry(
        "heat transfer dcdd stabilization", "false", Patterns::Bool(),
        "Apply Discontinuity-Capturing Directional Dissipation (DCDD) "
        "stabilization term on heat transfer <true|false>");

    prm.declare_entry(
        "vof dcdd stabilization", "true", Patterns::Bool(),
        "Apply Discontinuity-Capturing Directional Dissipation (DCDD) "
        "stabilization term on the VOF phase fraction <true|false>");

    prm.declare_entry(
        "pressure scaling factor", "1", Patterns::Double(),
        "This parameter can be used to change the scale of pressure in the "
        "Navier-Stokes equations. When the velocity and pressure scales are "
        "very "
        "different, using this parameter allows to reduce the condition number"
        " and reach a solution.");

    prm.declare_entry("scalar limiter", "none", Patterns::Selection("none|moe"),
                      "Type of scalar limiter. The limiters are only "
                      "appropriate with the DG versions of the solvers and "
                      "should only be used for advection-dominated problem.");
  }
  prm.leave_subsection();
}

void Stabilization::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("stabilization");
  {
...
void PhaseChange::parse_parameters(ParameterHandler &prm,
                                   const Dimensionality &dimensions) {
...
void PhaseChange::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("phase change");
  {
    prm.declare_entry("solidus temperature", "0", Patterns::Double(),
    ...
    prm.declare_entry("latent enthalpy", "1", Patterns::Double(),
                      "Enthalpy of the phase change");

    prm.declare_entry("specific heat liquid", "1", Patterns::Double(),
                      "Specific heat of the liquid phase");

    prm.declare_entry("specific heat solid", "1", Patterns::Double(),
                      "Specific heat of the solid phase");

    prm.declare_entry("thermal conductivity liquid", "1", Patterns::Double(),
                      "Thermal conductivity of the liquid phase");

    prm.declare_entry("thermal conductivity solid", "1", Patterns::Double(),
                      "Thermal conductivity of the solid phase");

    prm.declare_entry("thermal expansion liquid", "1", Patterns::Double(),
                      "Thermal expansion coefficient of the liquid phase");

    prm.declare_entry("thermal expansion solid", "0", Patterns::Double(),
                      "Thermal expansion coefficient of the solid phase");

    prm.declare_entry("viscosity liquid", "1", Patterns::Double(),
                      "Kinematic viscosity of the liquid phase");

    prm.declare_entry("viscosity solid", "1", Patterns::Double(),
                      "Kinematic viscosity of the solid phase");

    prm.declare_entry("Darcy penalty liquid", "0", Patterns::Double(),
                      "Darcy penalty of the liquid phase");

    prm.declare_entry("Darcy penalty solid", "0", Patterns::Double(),
...
void PhysicalProperties::declare_parameters(ParameterHandler &prm) {
  ...
  {
    prm.declare_entry("number of fluids", "1", Patterns::Integer(),
                      "Number of fluids");
    ...
    prm.declare_entry("number of solids", "0", Patterns::Integer(),
                      "Number of solids");
    ...
    prm.declare_entry("reference temperature", "0", Patterns::Double(),
                      "Reference temperature used for the calculation of "
                      "physical properties and thermal expansion");
  }

  // Definition of interactions between materials
  prm.declare_entry(
      "number of material interactions", "0", Patterns::Integer(),
...
void PhysicalProperties::parse_parameters(ParameterHandler &prm,
  ...
  prm.enter_subsection("physical properties");
  {
    ...
    AssertThrow(
        number_of_material_interactions <= max_material_interactions,
        NumberOfMaterialInteractionsError(number_of_material_interactions));
...
void Material::declare_parameters(ParameterHandler &prm,
  ...
  prm.enter_subsection(material_prefix + " " + Utilities::int_to_string(id, 1));
  {
    prm.declare_entry("density", "1", Patterns::Double(),
                      "Density for the fluid corresponding to Phase = " +
                          Utilities::int_to_string(id, 1));
    prm.declare_entry(
        "kinematic viscosity", "1", Patterns::Double(),
        "Kinematic viscosity for the fluid corresponding to Phase = " +
            Utilities::int_to_string(id, 1));
    prm.declare_entry("specific heat", "1", Patterns::Double(),
                      "Specific heat for the fluid corresponding to Phase = " +
                          Utilities::int_to_string(id, 1));
    prm.declare_entry(
        "thermal conductivity", "1", Patterns::Double(),
        "Thermal conductivity for the fluid corresponding to Phase = " +
            Utilities::int_to_string(id, 1));
    prm.declare_entry("thermal expansion", "1", Patterns::Double(),
                      "Thermal expansion coefficient for the fluid "
                      "corresponding to Phase = " +
                          Utilities::int_to_string(id, 1));

    prm.declare_entry(
        "tracer diffusivity model", "constant",
        Patterns::Selection(
    ...
    prm.declare_entry("density model", "constant",
    ...
    prm.declare_entry("specific heat model", "constant",
    ...
    prm.declare_entry(
        "thermal expansion model", "constant",
        Patterns::Selection("constant|phase_change"),
        "Model used for the calculation of the thermal expansion coefficient"
        "Choices are <constant|phase_change>.");

    prm.declare_entry("k_A0", "0", Patterns::Double(),
                      "k_A0 parameter for linear conductivity model");

    prm.declare_entry("k_A1", "0", Patterns::Double(),
...
void MaterialInteractions::declare_parameters(ParameterHandler &prm,
                                              const unsigned int id) const {
  prm.enter_subsection("material interaction " +
                       Utilities::int_to_string(id, 1));
  {
    prm.declare_entry("type", "fluid-fluid",
                      Patterns::Selection("fluid-fluid|fluid-solid"),
                      "Type of materials interacting. The choices are "
    ...
    {
      prm.declare_entry(
          "first fluid id", "0", Patterns::Integer(),
          "ID of the first fluid interacting with the second fluid. This value "
          "should be lower than the second fluid's id.");
      prm.declare_entry(
          "second fluid id", "1", Patterns::Integer(),
          "ID of the second fluid interacting with the first fluid. This value "
          "should be greater than the first fluid's id.");

      // Surface tension interactions
      prm.declare_entry(
          "surface tension model", "constant",
      ...
      prm.declare_entry("cahn hilliard mobility model", "constant",
                        Patterns::Selection("constant|quartic"),
                        "Model used for the calculation of the mobility in the "
    ...
    prm.enter_subsection("fluid-solid interaction");
    {
      ...
      prm.declare_entry("solid id", "0", Patterns::Integer(),
...
void MaterialInteractions::parse_parameters(
    ParameterHandler &prm, unsigned int id,
    const Parameters::Dimensionality &dimensions) {
  prm.enter_subsection("material interaction " +
                       Utilities::int_to_string(id, 1));
  {
    ...
    if (op == "fluid-fluid")
      ...
      throw(std::runtime_error("Invalid material interaction type. The choices "
                               "are <fluid-fluid|fluid-solid>."));
    ...
    if (material_interaction_type == MaterialInteractionsType::fluid_fluid) {
      prm.enter_subsection("fluid-fluid interaction");
      {
        ...
        if (op == "constant") {
          ...
          throw(std::runtime_error("Invalid surface tension model. The choices "
                                   "are <constant|linear|phase change>."));
    ...
    } else // Solid-fluid interactions
    {
      ...
      if (op == "constant") {
        ...
        AssertThrow(false,
                    ExcMessage("Invalid surface tension model. The choices are "
                               "<constant|linear|phase change>."));
...
void FEM::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("FEM");
  {
    ...
    prm.declare_entry("void fraction order", "1", Patterns::Integer(),
                      "interpolation order void fraction");
    prm.declare_entry("temperature order", "1", Patterns::Integer(),
                      "interpolation order temperature");
    prm.declare_entry("tracer order", "1", Patterns::Integer(),
                      "interpolation order tracer");
    prm.declare_entry("VOF order", "1", Patterns::Integer(),
                      "interpolation order tracer");
    prm.declare_entry(
        "phase cahn hilliard order", "1", Patterns::Integer(),
        "interpolation order phase parameter in the Cahn-Hilliard equations");
    prm.declare_entry("potential cahn hilliard order", "1", Patterns::Integer(),
                      "interpolation order chemical potential in the "
                      "Cahn-Hilliard equations");

    prm.declare_entry(
        "tracer uses dg", "false", Patterns::Bool(),
        "Switch tracer to Discontinuous Galerkin (DG) formulation");

    prm.declare_entry("VOF uses dg", "false", Patterns::Bool(),
                      "Switch VOF to Discontinuous Galerkin (DG) formulation");

    prm.declare_entry("enable bubble function velocity", "false",
                      Patterns::Bool(),
                      "Enable bubble enrichment function for velocity field");

    prm.declare_entry("enable bubble function pressure", "false",
                      Patterns::Bool(),
...
void Forces::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("forces");
  {
    ...
    prm.declare_entry("calculate force", "false", Patterns::Bool(),
                      "Enable calculation of force");
    prm.declare_entry("calculate torque", "false", Patterns::Bool(),
...
void Laser_FreeSurfaceRadiation::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("free surface radiation");
  {
    prm.declare_entry("enable", "false", Patterns::Bool(),
                      "Enable radiation at the free surface (air/metal "
                      "interface) <true|false>");
    prm.declare_entry("Stefan-Boltzmann constant", "5.6703e-8",
                      Patterns::Double(), "Stefan-Boltzmann constant");
    prm.declare_entry("emissivity", "0.6", Patterns::Double(),
                      "Emissivity of the free surface (air/metal interface)");
    prm.declare_entry("Tinf", "0.0", Patterns::Double(),
                      "Temperature (Double) of environment for radiation term "
                      "at the free surface (air/metal interface)");
  }
  prm.leave_subsection();
}

void Laser_FreeSurfaceRadiation::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("free surface radiation");
  {
...
template <int dim> void Laser<dim>::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("laser parameters");
  {
    prm.declare_entry("enable", "false", Patterns::Bool(), "Activate laser");
    prm.declare_entry(
        "type", "gaussian_heat_flux_vof_interface",
        Patterns::Selection("exponential_decay|gaussian_heat_flux_vof_"
                            "interface|uniform_heat_flux_vof_interface"),
        "Type of laser model used."
        "Choices are "
        "<exponential_decay|gaussian_heat_flux_vof_interface|uniform_heat_flux_"
        "vof_interface>.");
    prm.declare_entry("concentration factor", "2.0", Patterns::Double(),
                      "Concentration factor");
    prm.declare_entry("power", "0.0", Patterns::Double(), "Laser power");
    prm.declare_entry("absorptivity", "0.5", Patterns::Double(),
                      "Laser absorptivity");
    prm.declare_entry("penetration depth", "1.0", Patterns::Double(),
                      "Penetration depth");
    prm.declare_entry("beam radius", "0.0", Patterns::Double(),
                      "Laser beam radius");
    ...
    prm.declare_entry("start time", "0.0", Patterns::Double(),
                      "Start time of laser");
    prm.declare_entry("end time", "1.0", Patterns::Double(),
                      "End time of laser");

    prm.declare_entry("beam orientation", "z-",
                      Patterns::Selection("x+|x-|y+|y-|z+|z-"),
                      "Laser beam orientation "
                      "Choices are <x+|x-|y+|y-|z+|z->.");

    prm.declare_entry("beam rotation angle", "0.0", Patterns::Double(),
                      "Angle of rotation in rad of the beam axis with respect "
                      "to the axis defined by the beam orientation parameter");

    prm.declare_entry(
        "beam rotation axis", "0.0, 0.0, 1.0",
        Patterns::List(Patterns::Double()),
...
template <int dim> void Laser<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("laser parameters");
  {
    ...
    if constexpr (dim == 2)
      rotation_matrix = Physics::Transformations::Rotations::rotation_matrix_2d(
          rotation_angle);
    if constexpr (dim == 3)
      rotation_matrix = Physics::Transformations::Rotations::rotation_matrix_3d(
...
void PostProcessing::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("post-processing");
  {
    ...
    prm.declare_entry(
        "calculate kinetic energy", "false", Patterns::Bool(),
        "Enable calculation of total kinetic energy. The total kinetic "
        "energy is calculated from the volumetric integral of the kinetic "
        "energy over the domain.");

    prm.declare_entry(
        "calculate enstrophy", "false", Patterns::Bool(),
        "Enable calculation of total enstrophy. The total enstrophy "
        "is calculated from the volumetric integral of the enstrophy over the "
        "domain.");

    prm.declare_entry(
        "calculate pressure power", "false", Patterns::Bool(),
        "Enable calculation of the pressure power. The pressure power "
        "is calculated from the volumetric integral of u.grad(p) over the "
        "domain.");

    prm.declare_entry("calculate viscous dissipation", "false",
                      Patterns::Bool(),
                      "Enable calculation of the viscous dissipation. The "
                      "viscous dissipation "
                      "is calculated from the volumetric integral of "
                      "grad(u).tau over the domain.");

    prm.declare_entry("calculate apparent viscosity", "false", Patterns::Bool(),
                      "Enable calculation of apparent viscosity");

    prm.declare_entry("calculate average velocities", "false", Patterns::Bool(),
                      "Enable calculation of average velocities.");

    prm.declare_entry("calculate average temperature and heat flux", "false",
                      Patterns::Bool(),
                      "Enable calculation of time average temperature and time "
                      "average heat flux");

    prm.declare_entry(
        "calculate pressure drop", "false", Patterns::Bool(),
        "Enable calculation of pressure drop between two boundaries.");

    prm.declare_entry("inlet boundary id", "0", Patterns::Integer(),
                      "Inlet boundary ID for pressure drop calculation");

    prm.declare_entry("outlet boundary id", "1", Patterns::Integer(),
                      "Outlet boundary ID for pressure drop calculation");

    prm.declare_entry("calculate flow rate", "false", Patterns::Bool(),
                      "Enable calculation of flow rate at boundaries.");

    prm.declare_entry("calculate tracer flow rate", "false", Patterns::Bool(),
                      "Enable calculation of tracer flow rate at boundaries.");

    prm.declare_entry(
        "initial time for average velocity", "0.0", Patterns::Double(),
        "Initial time to start calculations for average velocities");

    prm.declare_entry(
        "initial time for average temperature and heat flux", "0.0",
        Patterns::Double(),
    ...
    prm.declare_entry("pressure drop name", "pressure_drop",
                      Patterns::FileName(), "File output pressure drop");

    prm.declare_entry("flow rate name", "flow_rate", Patterns::FileName(),
                      "File output volumetric flux");

    prm.declare_entry("tracer flow rate name", "tracer_flow_rate",
                      Patterns::FileName(),
    ...
    prm.declare_entry("pressure power name", "pressure_power",
                      Patterns::FileName(), "File output pressure power");

    prm.declare_entry("viscous dissipation name", "viscous_dissipation",
                      Patterns::FileName(), "File output viscous dissipation");

    prm.declare_entry("apparent viscosity name", "apparent_viscosity",
                      Patterns::FileName(), "File output apparent viscosity");

    prm.declare_entry("output frequency", "1", Patterns::Integer(),
                      "Output frequency");

    prm.declare_entry("calculate tracer statistics", "false", Patterns::Bool(),
                      "Enable calculation of tracer statistics.");

    prm.declare_entry("tracer statistics name", "tracer_statistics",
                      Patterns::FileName(),
                      "File name output tracer statistics");

    prm.declare_entry(
        "calculate phase statistics", "false", Patterns::Bool(),
        "Enable calculation of phase statistics: maximum, minimum, average and "
        "integral over the domain (Cahn-Hilliard).");

    prm.declare_entry("phase statistics name", "phase_statistics",
                      Patterns::FileName(),
                      "File name output phase statistics (Cahn-Hilliard)");

    prm.declare_entry("calculate temperature statistics", "false",
                      Patterns::Bool(),
                      "Enable calculation of temperature statistics.");

    prm.declare_entry("temperature statistics name", "temperature_statistics",
                      Patterns::FileName(),
                      "File name output temperature statistics");

    prm.declare_entry(
        "calculate liquid fraction", "false", Patterns::Bool(),
        "Enable calculation of the liquid fraction. The liquid fraction "
        "is calculated from the volume integral of the liquid fraction divided "
        "by the volume of the domain.");

    prm.declare_entry("liquid fraction name", "liquid_fraction",
                      Patterns::FileName(), "File name output liquid fraction");

    prm.declare_entry("calculate heat flux", "false", Patterns::Bool(),
                      "Enable calculation of heat flux.");

    prm.declare_entry("heat flux name", "heat_flux", Patterns::FileName(),
                      "File name output for the heat flux");

    prm.declare_entry("convective flux name", "convective_flux",
                      Patterns::FileName(),
                      "File name output for the convective flux");

    prm.declare_entry("nitsche flux name", "nitsche_heat_flux",
                      Patterns::FileName(),
                      "File name output for the convective flux");

    prm.declare_entry("postprocessed fluid", "both",
    ...
    prm.declare_entry(
        "calculate barycenter", "false", Patterns::Bool(),
        "Enable calculation of the barycenter location and velocity of fluid 1 "
        "in VOF and Cahn-Hilliard simulations.");

    prm.declare_entry("barycenter name", "barycenter_information",
                      Patterns::FileName(),
                      "Name of barycenter information output file in VOF or "
                      "Cahn-Hilliard simulations");

    prm.declare_entry("calculate mass conservation", "true", Patterns::Bool(),
                      "Enable calculation of the mass and momentum of both "
                      "fluids in VOF simulations.");

    prm.declare_entry(
        "mass conservation name", "mass_conservation_information",
        Patterns::FileName(),
        "Name of mass conservation output file in VOF simulations");

    prm.declare_entry("calculate phase energy", "false", Patterns::Bool(),
                      "Enable calculation of phase energies, including: total "
                      "energy, bulk energy, and interface energy");

    prm.declare_entry("phase energy name", "phase_energy", Patterns::FileName(),
                      "Name of energy output file in Cahn-Hilliard "
                      "simulations. The file is stored in the output folder "
                      "specified in the simulation control subsection");

    prm.declare_entry(
        "calculate phase volumes", "false", Patterns::Bool(),
        "Enable calculation of total volume each phases in cfd-dem simulation, "
        "including: total volume of fluid, and total volume of particles");

    prm.declare_entry("phase volumes name", "phase_volumes",
                      Patterns::FileName(),
                      "Name of phases volume output file in cfd-dem "
                      "simulations. The file is stored in the output folder "
                      "specified in the simulation control subsection");
  }
  prm.leave_subsection();
...
void PostProcessing::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("post-processing");
  {
    ...
    if (op_fluid == "fluid 1")
      ...
      throw(std::runtime_error("Invalid postprocessed fluid. "
  ...
  prm.enter_subsection("non-linear solver");
  {
    prm.enter_subsection(physics_name);
    {
      prm.declare_entry("verbosity", "verbose",
                        ...
                        "State whether the outputs from the non-linear solver "
                        "should be printed. "
                        "Choices are <quiet|verbose>.");
      ...
      prm.declare_entry(
          ...
          "an analytical jacobian formulation. The jacobian matrix and the "
          "preconditioner"
          "are assembled every iteration. In the kinsol_newton method, the "
          "nonlinear solver"
          "Kinsol from the SUNDIALS library is used. This solver has an "
          "internal algorithm"
          "that decides whether to reassemble the Jacobian matrix or not.");

      prm.declare_entry(
          "kinsol strategy", "line_search",
      ...
      prm.declare_entry(
          "step tolerance", "0.9", Patterns::Double(),
      ...
      prm.declare_entry(
          "matrix tolerance", "0.1", Patterns::Double(),
          "This parameter controls the frequency at which the matrix is "
          "refreshed in the inexact Newton solvers"
          "If the residual after a newton step < previous residual * matrix "
          "tolerance, the matrix is not re-assembled");

      prm.declare_entry(
          "force rhs calculation", "false", Patterns::Bool(),
          "This is required if there is a fixed point component to the "
          "non-linear"
      ...
      prm.declare_entry("reuse matrix", "false", Patterns::Bool(),
                        "Reuse the last jacobian matrix for the next "
                        "non-linear problem solution");

      prm.declare_entry("reuse preconditioner", "false", Patterns::Bool(),
                        "Reuse the last preconditioner for the next non-linear "
                        "problem solution");

      prm.declare_entry("abort at convergence failure", "false",
                        Patterns::Bool(),
                        "Aborts Lethe by throwing an exception if non-linear "
                        "solver convergence has failed");
    }
    prm.leave_subsection();
  }
  prm.leave_subsection();
...
void Mesh::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("mesh");
  {
    prm.declare_entry(
        "type", "dealii",
        Patterns::Selection("gmsh|dealii|periodic_hills|cylinder"),
        "Type of mesh "
    ...
    prm.declare_entry("initial boundary refinement", "0", Patterns::Integer(),
                      "Initial refinement of the mesh at the boundaries "
    ...
    prm.declare_entry(
        "enable target size", "false", Patterns::Bool(),
        "Enable initial refinement until target size is reached.");

    prm.declare_entry("simplex", "false", Patterns::Bool(),
                      "Indicates that the mesh used is a mesh made of only "
                      "simplex elements.");

    prm.declare_entry(
        "check diamond cells", "false", Patterns::Bool(),
        "Enables checking the input grid for diamond-shaped cells.");

    prm.declare_entry(
        "expand particle-wall contact search", "false", Patterns::Bool(),
    ...
    prm.declare_entry("target size", "1", Patterns::Double(),
    ...
    prm.declare_entry(
        "initial translation", "0, 0, 0", Patterns::List(Patterns::Double()),
        "Component of the desired translation of the mesh at initialization");

    prm.declare_entry(
        "initial rotation axis", "1, 0, 0", Patterns::List(Patterns::Double()),
        "Component of the desired rotation of the mesh at initialization");

    prm.declare_entry("initial rotation angle", "0", Patterns::Double(),
                      "Angle of rotation of the mesh at initialization around "
                      "the axis in radian");

    prm.declare_entry("scale", "1", Patterns::Double(),
...
void Mesh::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("mesh");
  {
    ...
    // Initial rotation axis and angle
    rotation_axis = value_string_to_tensor<3>(prm.get("initial rotation axis"));
...
void MeshBoxRefinement::declare_parameters(ParameterHandler &prm) {
...
void MeshBoxRefinement::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("box refinement");
  {
...
void LinearSolver::declare_parameters(ParameterHandler &prm,
  ...
  prm.enter_subsection("linear solver");
  {
    prm.enter_subsection(physics_name);
    {
      ...
      prm.declare_entry(
          "method", "gmres", Patterns::Selection("gmres|bicgstab|direct"),
          "The iterative solver for the linear system of equations. "
          "Choices are <gmres|bicgstab|direct>.");

      prm.declare_entry("rescale residual", "false", Patterns::Bool(),
                        "Rescale the residual by the square root of the volume "
                        "of the triangulation");
      prm.declare_entry("relative residual", "1e-3", Patterns::Double(),
                        "Linear solver residual");
      prm.declare_entry("minimum residual", "1e-12", Patterns::Double(),
                        "Linear solver minimum residual");
      prm.declare_entry("max iters", "1000", Patterns::Integer(),
                        "Maximum solver iterations");

      prm.declare_entry("max krylov vectors", "100", Patterns::Integer(),
                        "Maximum number of krylov vectors for GMRES");

      prm.declare_entry(
          "enable hessians in jacobian", "true", Patterns::Bool(),
          "Turns off the terms involving the hessian in the Jacobian");

      prm.declare_entry("enable hessians in residual", "true", Patterns::Bool(),
      ...
      prm.declare_entry("ilu preconditioner fill", "0", Patterns::Double(),
                        "Ilu preconditioner fill");

      prm.declare_entry("ilu preconditioner absolute tolerance", "1e-12",
      ...
      prm.declare_entry("amg preconditioner ilu fill", "0", Patterns::Double(),
      ...
      prm.declare_entry("amg aggregation threshold", "1e-14",
                        Patterns::Double(), "amg aggregation threshold");
      ...
      prm.declare_entry("force linear solver continuation", "false",
                        Patterns::Bool(),
                        "A boolean that will force the linear solver to "
                        "continue even if it fails");

      prm.declare_entry("mg min level", "-1", Patterns::Integer(),
                        "mg min level");

      prm.declare_entry("mg level min cells", "-1", Patterns::Integer(),
                        "mg minimum number of cells for coarse level");

      prm.declare_entry("mg int level", "-1", Patterns::Integer(),
                        "mg int level");

      prm.declare_entry("mg enable hessians in jacobian", "true",
                        Patterns::Bool(),
                        "Turns off the terms involving the hessian in the "
                        "Jacobian of mg operators");

      prm.declare_entry("mg smoother iterations", "10", Patterns::Integer(),
                        "mg smoother iterations for lsmg or gcmg");

      prm.declare_entry("mg smoother relaxation", "0.5", Patterns::Double(),
                        "mg smoother relaxation for lsmg or gcmg");

      prm.declare_entry(
          "mg smoother preconditioner type", "inverse diagonal",
          Patterns::Selection("inverse diagonal|additive schwarz method"),
          "Preconditioner of smoother");

      prm.declare_entry("mg smoother eig estimation", "true", Patterns::Bool(),
                        "estimate eigenvalues for relaxation parameter");

      prm.declare_entry("eig estimation smoothing range", "10",
                        Patterns::Double(),
                        "sets range between largest and smallest eig");

      prm.declare_entry("eig estimation cg n iterations", "10",
                        Patterns::Integer(),
                        "cg iterations performed to find eigenvalue");

      prm.declare_entry("eig estimation verbosity", "verbose",
                        Patterns::Selection("quiet|verbose"),
                        "State whether MG should print max and min eigenvalue"
                        "Choices are <quiet|verbose>.");

      prm.declare_entry("mg coarse grid solver", "direct",
                        Patterns::Selection("gmres|amg|ilu|direct"),
                        "The coarse grid solver for lsmg or gcmg"
                        "Choices are <gmres|amg|ilu|direct>.");

      prm.declare_entry(
          "mg coarse grid use fe q iso q1", "false", Patterns::Bool(),
          "use elements with linear interpolation for coarse grid");

      prm.declare_entry("mg coarsening type", "h",
                        Patterns::Selection("h|p|hp|ph"),
                        "mg coarsening type for gcmg");

      prm.declare_entry("mg p coarsening type", "decrease by one",
                        Patterns::Selection("decrease by one|bisect|go to one"),
                        "mg p coarsening type for gcmg");

      prm.declare_entry("mg gmres max iterations", "2000", Patterns::Integer(),
                        "mg gmres iterations for lsmg or gcmg");

      prm.declare_entry("mg gmres tolerance", "1e-14", Patterns::Double(),
                        "mg gmres tolerance n for lsmg or gcmg");

      prm.declare_entry("mg gmres reduce", "1e-4", Patterns::Double(),
                        "mg gmres reduce for lsmg or gcmg");

      prm.declare_entry("mg gmres max krylov vectors", "30",
                        Patterns::Integer(),
                        "mg gmres max krylov vectors for lsmg or gcmg");

      prm.declare_entry("mg gmres preconditioner", "amg",
                        Patterns::Selection("amg|ilu"),
                        "The preconditioner for the mg gmres solver"
                        "Choices are <amg|ilu>.");

      prm.declare_entry("mg amg use default parameters", "false",
                        Patterns::Bool(),
                        "Use default parameters for Trilinos AMG");

      prm.declare_entry(
          "mg verbosity", "verbose",
          Patterns::Selection("quiet|verbose|extra verbose"),
  ...
  prm.enter_subsection("linear solver");
  {
    prm.enter_subsection(physics_name);
    {
      ...
      if (sv == "gmres")
        ...
        throw std::logic_error("Error, invalid iterative solver type. Choices "
                               "are amg, gmres, bicgstab or direct");
      ...
      if (precond == "amg")
        ...
        throw std::logic_error("Error, invalid preconditioner type. Choices "
                               "are amg, ilu, lsmg or gcmg.");
      ...
      else if (mg_smoother_preconditioner_type_str == "additive schwarz method")
        this->mg_smoother_preconditioner_type =
      ...
      const std::string eig_estimation_v = prm.get("eig estimation verbosity");
      ...
      if (cg_solver == "gmres")
        ...
        throw std::logic_error("Error, invalid coarse grid solver type. "
                               "Choices are gmres, amg, ilu or direct.");

      mg_use_fe_q_iso_q1 = prm.get_bool("mg coarse grid use fe q iso q1");

      const std::string mg_coarsening_type_str = prm.get("mg coarsening type");
      ...
      AssertThrow((!mg_use_fe_q_iso_q1) || (this->mg_coarsening_type ==
      ...
      if (cg_precond == "amg")
        ...
        throw std::logic_error("Error, invalid preconditioner type for mg "
                               "gmres solver. Choices are amg or ilu.");
...
void MeshAdaptation::declare_parameters(ParameterHandler &prm) {
  ...
  {
    prm.declare_entry("initial refinement steps", "0", Patterns::Integer(),
    ...
    prm.declare_entry("fraction refinement", "0.1",
                      ...
                      "For multi-variables refinement, separate the different "
                      "fractions with a comma "
    ...
    prm.declare_entry("variable", "velocity",
                      Patterns::List(Patterns::Selection(
                          "velocity|pressure|phase|temperature|phase_cahn_"
                          "hilliard|chemical_potential_cahn_hilliard|tracer")),
    ...
    prm.declare_entry(
        "mesh refinement controller", "false", Patterns::Bool(),
        "Fraction of refined elements"
        "Enable a controller that will target a specific number of elements in "
        "the mesh equal to the maximum number of elements");
...
void MeshAdaptation::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("mesh adaptation");
  {
    ...
    std::vector<std::string> coars_vec = Utilities::split_string_list(coars_op);
    const std::string refin_op = prm.get("fraction refinement");
    std::vector<std::string> refin_vec = Utilities::split_string_list(refin_op);

    // Checking that the sizes are coherent
    Assert(coars_vec.size() == var_vec.size(),
           MultipleAdaptationSizeError("fraction coarsening", coars_vec.size(),
                                       var_vec.size()));
    Assert(refin_vec.size() == var_vec.size(),
           MultipleAdaptationSizeError("fraction refinement", refin_vec.size(),
                                       var_vec.size()));
    ...
    for (std::vector<int>::size_type i = 0; i != var_vec.size(); ++i) {
      ...
      if (var_vec[i] == "velocity")
        ...
        throw std::logic_error(
            "Error, invalid mesh adaptation variable. Choices are velocity, "
            "pressure, phase, temperature, phase_cahn_hilliard, "
            "chemical_potential_cahn_hilliard or tracer");
...
void Testing::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("test");
  {
    prm.declare_entry(
        ...
        "testing mode is generally used only for the automatic testing bench "
        "using ctest.");
    prm.declare_entry(
        "type", "particles",
        Patterns::Selection("particles|mobility_status|subdomain"),
        "Output type for testing mode. Currently, particles type will output "
        "each particle with some information and mobility_status or subdomain "
        "output results "
        "in deal.II format.");
  }
  prm.leave_subsection();
...
void VelocitySource::declare_parameters(ParameterHandler &prm) {
  ...
  {
    prm.declare_entry("rotating frame type", "none",
                      ...
                      "the coriolis and the centrifugal force to the "
    ...
    prm.declare_entry("Darcy type", "none",
...
template <int dim>
void IBParticles<dim>::declare_default_entry(ParameterHandler &prm,
                                             unsigned int index) {
  prm.declare_entry(
      "integrate motion", "false", Patterns::Bool(),
      "Bool to define if the particle trajectory is integrated meaning its "
      "velocity and position will be updated at each time step according to "
      "the hydrodynamic force applied to it");
  prm.declare_entry(
      "mesh-based precalculations", "true", Patterns::Bool(),
      "Bool to define if precalculations should be performed between "
      "refinements. Precalculations can introduce shape deformation when the "
      "type is RBF and some nodes are located outside the background mesh.");
  ...
  prm.declare_entry(
      "type", "sphere",
      Patterns::Selection(
          "sphere|hyper rectangle|ellipsoid|torus|cone|cylinder|cylindrical "
          "tube|cylindrical helix|cut hollow sphere|death "
          "star|superquadric|rbf|opencascade|plane|composite"),
      "The type of shape considered."
      "Choices are <sphere|hyper "
      "rectangle|ellipsoid|torus|cone|cylinder|cylindrical tube|cylindrical "
      "helix|cut hollow sphere|death "
      "star|superquadric|rbf|opencascade|composite>."
  ...
  prm.declare_entry("shape arguments", "1", Patterns::Anything(),
                    "Arguments defining the geometry");

  prm.declare_entry("layer thickening", "0", Patterns::Double(),
                    "Thickness (positive or negative) of uniform additional "
                    "layer of solid on particle."
                    "A negative value will decrease the particle's thickness "
                    "by subtracting a layer of specified width.");

  prm.declare_entry(
      "pressure location", "0; 0; 0", Patterns::Anything(),
      "position relative to the center of the particle for the location of the "
      "point where the pressure is imposed inside the particle");
  ...
  prm.enter_subsection("physical properties");
  {
    ...
    prm.declare_entry(
        "volume", "0", Patterns::Double(),
        "The volume occupied by the particle. If it is left empty, the volume "
        "is automatically calculated if possible otherwise the volume of a "
        "sphere is used instead");
    prm.declare_entry("inertia", "1 ;0 ;0 ;0 ;1 ;0 ;0 ;0 ;1",
                      Patterns::Anything(),
                      "Moments of inertia of the particle in the reference "
                      "frame of the fluid. The entry sequence corresponds to : "
                      "I_xx ;I_xy ;I_xz ;I_yx ;I_yy ;I_yz ;I_zx ;I_zy ;I_zz");
    prm.declare_entry("youngs modulus", "100000000", Patterns::Double(),
    ...
    prm.declare_entry(
        "restitution coefficient", "1", Patterns::Double(),
        "The restitution coefficient of particle in case of contact");
    prm.declare_entry(
        "friction coefficient", "0", Patterns::Double(),
        "The friction coefficient of particle in case of contact");
    prm.declare_entry(
        "rolling friction coefficient", "0", Patterns::Double(),
...
template <int dim>
void IBParticles<dim>::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("particles");
  {
    prm.declare_entry(
        "number of particles", "1", Patterns::Integer(),
        "The number of particles represented by IB. The maximal number of "
        "particles is equal to 10 when defined individually. If particles are "
        "loaded from a file, this parameter is overridden, and there is no "
        "limit to the number of particles.");

    prm.declare_entry("assemble Navier-Stokes inside particles", "false",
                      Patterns::Bool(),
                      "Bool to define if you assemble the inside of particles "
                      "with the NS equation.");

    prm.enter_subsection("extrapolation function");
    {
      prm.declare_entry(
          "stencil order", "2", Patterns::Integer(),
          "The polynomial order used in the extrapolation function");
      prm.declare_entry(
          "length ratio", "4", Patterns::Double(),
          "The length ratio used to define the points for the IB stencil. See "
          "definition of epsilon_n in the paper on sharp IB.");
      prm.declare_entry(
          "enable extrapolation", "true", Patterns::Bool(),
          "Bool to define if extrapolation should be enabled (default). If "
          "disabled, all velocity degrees of freedom "
          "in a cell will be set to the particle velocity if that cell is cut. "
          "Setting to false is intended for "
          "debugging purposes.");
    ...
    {
      prm.declare_entry(
          "load particles from file", "false", Patterns::Bool(),
          "Bool to define if particles are loaded from an external file");
      prm.declare_entry("particles file", "particles", Patterns::FileName(),
    ...
    {
      prm.declare_entry("initial refinement", "0", Patterns::Integer(),
                        "Number of refinements around the particles before the "
                        "start of the simulation ");
      prm.declare_entry("refine mesh inside radius factor", "0.5",
                        Patterns::Double(),
                        "The factor that multiplies the radius to define the "
                        "inside bound for the refinement of the mesh");
      prm.declare_entry("refine mesh outside radius factor", "1.5",
                        Patterns::Double(),
                        "The factor that multiplies the radius to define the "
                        "outside bound for the refinement of the mesh");
      prm.declare_entry(
          "refinement zone extrapolation", "false", Patterns::Bool(),
          "This parameter enables the extrapolation in time of the refinement "
          "zone. This means that it will try to refine where the particle will "
          "be at the end of the time step instead of the initial position.");
      prm.leave_subsection();
    }
    ...
    {
      prm.declare_entry(
          "calculate force", "true", Patterns::Bool(),
          "Bool to define if the force is evaluated on each particle ");
      prm.declare_entry("ib force output file", "ib_force",
                        Patterns::FileName(),
                        "The name of the file where the data on the force of "
                        "each particle is stored");
      prm.declare_entry(
          "ib particles pvd file", "ib_particles_data", Patterns::FileName(),
          "The output files of the pvd data for the ib particles");
      prm.declare_entry(
          "print DEM", "true", Patterns::Bool(),
          "Bool to define if particles' information are printed on the "
          "terminal when particles' time-step is finished");
      prm.declare_entry("enable extra sharp interface vtu output field",
                        "false", Patterns::Bool(),
                        "This parameter enables the output of more information "
                        "related to the particles in the vtu file.");
      prm.leave_subsection();
    }

    prm.enter_subsection("DEM");
    {
      prm.declare_entry("contact search radius factor", "3", Patterns::Double(),
                        "The factor that multiplies the radius to define the "
                        "region of contact search around the particle");
      prm.declare_entry(
          "contact search frequency", "1", Patterns::Integer(),
          "The frequency of update in the contact candidates list");
      prm.declare_entry("particle nonlinear tolerance", "1e-6",
                        Patterns::Double(),
                        "The nonlinear tolerance for the coupling of the "
                        "particle dynamics and the fluid");
      prm.declare_entry("DEM coupling frequency", "1000", Patterns::Integer(),
                        "The number of DEM time steps per CFD time step");
      prm.declare_entry("alpha", "1", Patterns::Double(),
                        "relaxation parameter");

      prm.declare_entry("enable lubrication force", "true", Patterns::Bool(),
                        "Bool to enable or disable the lubrication force");
      prm.declare_entry("lubrication range max", "2", Patterns::Double(),
                        "Gap require to consider the lubrication force. This "
                        "value is multiplied the smallest cell size");
      prm.declare_entry(
          "lubrication range min", "0.1", Patterns::Double(),
          "Smallest gap considered for the lubrification force calculation. "
          "This value is multiplied by the smallest cell size");

      prm.declare_entry(
          "explicit contact impulsion", "false", Patterns::Bool(),
          "Bool to enable or disable the use of explicit contact impulsion "
          "evaluation in the resolution of the coupling of the particle. When "
          "it is set to true, this parameter results in the code only "
          "performing the DEM calculation once per CFD time step and using the "
          "resulting contact impulsion to evaluate all the other Newton's "
          "iterations. This reduces the number of times the DEM calculation is "
          "made.");

      prm.declare_entry(
          "explicit position integration", "false", Patterns::Bool(),
          "Bool to enable or disable the explicit position integration. This "
          "means that the particle position is obtained directly by the "
          "integration of the previous velocities only. This avoids multiple "
          "cut cell mapping for each newton iteration. Note that this limits "
          "the order of convergence in time to one.");

      prm.declare_entry(
          "approximate radius for contact", "false", Patterns::Bool(),
          "Bool to turn on or off using the approximate radius of the "
          "particles during contact. If activated, the radius used in the "
          "contact calculation is constant and fixed to the effective radius "
          "of the shape. If not, the radius of curvature of the shape at the "
          "contact point is evaluated. For some shapes, this can be "
          "numerically expensive to evaluate.");

      prm.enter_subsection("wall physical properties");
      {
        prm.declare_entry(
            "wall youngs modulus", "100000000", Patterns::Double(),
            "The wall Young's modulus if IB particles are in contact with it");

        prm.declare_entry(
            "wall poisson ratio", "0.3", Patterns::Double(),
            "The wall poisson ratio if IB particles are in contact with it");

        prm.declare_entry("wall rolling friction coefficient", "0",
                          Patterns::Double(),
                          "The wall rolling friction coefficient if IB "
                          "particles are in contact with it");

        prm.declare_entry("wall friction coefficient", "0", Patterns::Double(),
                          "The wall friction coefficient if IB particles are "
                          "in contact with it");

        prm.declare_entry("wall restitution coefficient", "1",
                          Patterns::Double(),
                          "The wall restitution coefficient if IB particles "
...
template <int dim>
void IBParticles<dim>::parse_parameters(ParameterHandler &prm) {
  using numbers::PI;
  prm.enter_subsection("particles");
  {
    prm.enter_subsection("extrapolation function");
    ...
    prm.enter_subsection("DEM");
    {
      ...
      if (contact_search_radius_factor < 1.)
        throw(std::logic_error("Error, the parameter 'contact search radius "
                               "factor' cannot be < 1."));
    ...
    for (unsigned int i = 0; i < nb_particles; ++i) {
      ...
      prm.enter_subsection("physical properties");
      {
        ...
        if (inertia_str_list.size() == 9) {
          ...
          throw(std::runtime_error(
              " Invalid inertia matrix. The inertia is given as a 3 by 3 "
              "matrix or a single value if the inertia is uniform around each "
              "axis."));
        }
...
void DynamicFlowControl::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("flow control");
  {
    prm.declare_entry("enable", "false", Patterns::Bool(),
                      "Enable flow rate control");
    prm.declare_entry("average velocity", "0", Patterns::Double(),
                      "The target average velocity");
    prm.declare_entry("inlet boundary id", "0", Patterns::Integer(),
                      "Boundary id of the inlet flow");
    prm.declare_entry("flow direction", "0", Patterns::Integer(),
                      "Flow direction at flow inlet");
    prm.declare_entry("initial beta", "0", Patterns::Double(),
                      "Beta coefficient value for the first step time");
    prm.declare_entry("alpha", "1", Patterns::Double(),
                      "Relaxation coefficient for flow controller");
    prm.declare_entry("enable beta particle", "false", Patterns::Bool(),
                      "Enable beta force for particles");
    prm.declare_entry("beta threshold", "0.0", Patterns::Double(),
                      "Enable beta force for particles");
    prm.declare_entry(
        "verbosity", "quiet", Patterns::Selection("quiet|verbose"),
...
void DynamicFlowControl::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("flow control");
  {
    // Enable flow control
...
void Evaporation::declare_parameters(dealii::ParameterHandler &prm) {
  prm.enter_subsection("evaporation");
  {
    prm.declare_entry(
        "evaporation mass flux model", "constant",
    ...
    prm.declare_entry(
        "enable evaporative cooling", "false", Patterns::Bool(),
        "Enable the evaporative cooling at the free surface (gas/liquid "
        "interface) in the energy equation <true|false>");
    prm.declare_entry(
        "enable recoil pressure", "false", Patterns::Bool(),
        "Enable the recoil pressure due to evaporation at the free surface "
        "(gas/liquid interface) in the momentum equation <true|false>");
    prm.declare_entry("evaporation mass flux", "0.0", Patterns::Double(),
                      "Evaporation mass flux used if the constant evaporation "
                      "model is selected in M*L^-2*T^-1");
    prm.declare_entry("evaporation coefficient", "0.82", Patterns::Double(),
                      "Evaporation coefficient corresponding to the ratio "
                      "between the net mass flux (evaporation-condensation) "
                      "and the mass flux of evaporation");
    prm.declare_entry("recoil pressure coefficient", "0.56", Patterns::Double(),
                      "Recoil pressure coefficient corresponding to the factor "
                      "applied to the saturation pressure to compute the "
                      "recoil pressure in an out of equilibrium evaporation");
    prm.declare_entry("molar mass", "1.0", Patterns::Double(),
                      "Molar mass of the material in M*N^-1");
    prm.declare_entry("boiling temperature", "1.0", Patterns::Double(),
                      "Boiling temperature in Theta");
    prm.declare_entry("evaporation latent heat", "0.0", Patterns::Double(),
                      "Latent heat of evaporation in L^2*T^-2");
    prm.declare_entry("ambient pressure", "101325", Patterns::Double(),
                      "Pressure of the ambient gas in M*L^-1*T^-2");
    prm.declare_entry("ambient gas density", "1.0", Patterns::Double(),
                      "Ambient gas density in M*L^-3");
    prm.declare_entry("liquid density", "10.0", Patterns::Double(),
                      "Liquid density in M*L^-3");
    prm.declare_entry("universal gas constant", "8.3145", Patterns::Double(),
                      "Universal gas constant in M*L^2*T^-2*Theta^-1*N^-1");
  }
  prm.leave_subsection();
}

void Evaporation::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("evaporation");
  {
    std::string op;
    ...
    if (op == "constant") {
      ...
      throw(
          std::runtime_error("Invalid evaporative mass flux model. The choices "
                             "are <constant|temperature_dependent>."));
    ...
    recoil_pressure_coefficient = prm.get_double("recoil pressure coefficient");
...
template <int dim> void Mortar<dim>::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("mortar");
  {
    ...
    prm.declare_entry("rotor boundary id", "1", Patterns::Integer(),
                      "Rotor boundary ID # of the mortar matching interface");
    prm.declare_entry("stator boundary id", "2", Patterns::Integer(),
                      "Stator boundary ID # of the mortar matching interface");
    std::string default_entry_point = (dim == 2) ? "0., 0." : "0., 0., 0.";
    prm.declare_entry("center of rotation", default_entry_point,
                      Patterns::List(Patterns::Double()),
                      "Center of rotation coordinates of rotor domain");

    prm.declare_entry("rotation axis", "0.0, 0.0, 1.0",
                      Patterns::List(Patterns::Double()),
    ...
    rotor_angular_velocity = std::make_shared<Functions::ParsedFunction<dim>>();
    rotor_angular_velocity->declare_parameters(prm);
    prm.leave_subsection();

    prm.declare_entry("penalty factor", "1.", Patterns::Double(),
                      "Penalty factor for mortar elements");
    prm.declare_entry("oversampling factor", "1", Patterns::Integer(),
...
template <int dim> void Mortar<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("mortar");
  {
