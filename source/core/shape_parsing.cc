...
template <int dim>
std::shared_ptr<Shape<dim>> ShapeGenerator::initialize_shape(
    const std::string &type, const std::string &shape_arguments_str,
  ...
  if (type == "rbf" || type == "composite" || type == "opencascade") {
    shape = initialize_shape_from_file(type, shape_arguments_str, position,
                                       orientation);
  } else {
    ...
    shape = initialize_shape_from_vector(type, shape_arguments, position,
                                         orientation);
  }
  return shape;
}
...
template <int dim>
std::shared_ptr<Shape<dim>> ShapeGenerator::initialize_shape_from_vector(
    const std::string &type, const std::vector<double> &shape_arguments,
    const Point<dim> &position, const Tensor<1, 3> &orientation) {
  std::shared_ptr<Shape<dim>> shape;
  if (type == "sphere")
    shape = std::make_shared<Sphere<dim>>(shape_arguments[0], position,
                                          orientation);
    ...
    shape = std::make_shared<HyperRectangle<dim>>(half_lengths, position,
                                                  orientation);
    ...
    if constexpr (dim == 3)
      shape = std::make_shared<Torus<dim>>(
          shape_arguments[0], shape_arguments[1], position, orientation);
  } else if (type == "cone") {
    if constexpr (dim == 3)
      shape = std::make_shared<Cone<dim>>(
          shape_arguments[0], shape_arguments[1], position, orientation);
  } else if (type == "cylinder") {
    if constexpr (dim == 3)
      shape = std::make_shared<Cylinder<dim>>(
          shape_arguments[0], shape_arguments[1], position, orientation);
  } else if (type == "cylindrical tube") {
    if constexpr (dim == 3)
      shape = std::make_shared<CylindricalTube<dim>>(
          shape_arguments[0], shape_arguments[1], shape_arguments[2], position,
          orientation);
  } else if (type == "cylindrical helix") {
    if constexpr (dim == 3)
      shape = std::make_shared<CylindricalHelix<dim>>(
          shape_arguments[0], shape_arguments[1], shape_arguments[2],
          shape_arguments[3], position, orientation);
  } else if (type == "cut hollow sphere") {
    if constexpr (dim == 3)
      shape = std::make_shared<CutHollowSphere<dim>>(
          shape_arguments[0], shape_arguments[1], shape_arguments[2], position,
          orientation);
  } else if (type == "death star") {
    if constexpr (dim == 3)
      shape = std::make_shared<DeathStar<dim>>(
          shape_arguments[0], shape_arguments[1], shape_arguments[2], position,
          orientation);
  } else if (type == "plane") {
...
template <int dim>
std::shared_ptr<Shape<dim>> ShapeGenerator::initialize_shape_from_file(
    const std::string &type, const std::string &file_name,
    const Point<dim> &position, const Tensor<1, 3> &orientation) {
  std::shared_ptr<Shape<dim>> shape;
  std::vector<double> shape_arguments;
  if (type == "rbf") {
    ...
    if (myfile) {
      ...
      while (std::getline(myfile, line)) {
        ...
        if (line == "shapes") {
          ...
          if (parsing_shapes) {
            ...
            const std::string &orientation_str = list_of_words_clean[4];
            ...
            Point<3> temp_orientation =
                Point<3>({temp_orientation_vec[0], temp_orientation_vec[1],
            ...
            if (type_str == "union") {
              operations[identifier] =
                  std::make_tuple(CompositeShape<dim>::BooleanOperation::Union,
      ...
      myfile.close();
      shape = std::make_shared<CompositeShape<dim>>(components, operations,
                                                    position, orientation);
    } else
      throw std::invalid_argument(file_name);
  } else if (type == "opencascade") {
    shape = std::make_shared<OpenCascadeShape<dim>>(file_name, position,
                                                    orientation);
  }
  return shape;
}

template std::shared_ptr<Shape<2>> ShapeGenerator::initialize_shape(
    const std::string &type, const std::string &arguments,
    const Point<2> &position, const Tensor<1, 3> &orientation);
template std::shared_ptr<Shape<3>> ShapeGenerator::initialize_shape(
    const std::string &type, const std::string &arguments,
    const Point<3> &position, const Tensor<1, 3> &orientation);
template std::shared_ptr<Shape<2>> ShapeGenerator::initialize_shape_from_vector(
    const std::string &type, const std::vector<double> &shape_arguments,
    const Point<2> &position, const Tensor<1, 3> &orientation);
template std::shared_ptr<Shape<3>> ShapeGenerator::initialize_shape_from_vector(
    const std::string &type, const std::vector<double> &shape_arguments,
    const Point<3> &position, const Tensor<1, 3> &orientation);
template std::shared_ptr<Shape<2>> ShapeGenerator::initialize_shape_from_file(
    const std::string &type, const std::string &file_name,
    const Point<2> &position, const Tensor<1, 3> &orientation);
template std::shared_ptr<Shape<3>> ShapeGenerator::initialize_shape_from_file(
    const std::string &type, const std::string &file_name,
    const Point<3> &position, const Tensor<1, 3> &orientation);
