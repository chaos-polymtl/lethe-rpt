...
TableHandler make_table_scalars_tensors(
  ...
  for (auto &vect : dependent_vectors) {
    ...
    for (unsigned int i = 0; i < vect.size(); ++i) {
      for (int d = 0; d < dim; ++d) {
        table.add_value(dependent_column_names[d + vect_index], vect[i][d]);
...
template TableHandler make_table_scalars_vectors(
...
template TableHandler make_table_scalars_tensors(
    const std::vector<double> &independent_values,
    const std::string &independent_column_name,
...
template TableHandler make_table_tensors_tensors(
...
template TableHandler make_table_tensors_scalars(
...
int get_max_value_of_parameter(const std::string &file_name,
                               const std::string &parameter_name) {
  ...
  if (dimension.size() > 0) {
    ...
    AssertThrow(
        ...
        dealii::ExcMessage(
            ...
            "Lethe requires that the user explicitly specify the dimension of "
            "the problem within the parameter file. This can be achieved by "
            "adding set dimension = 2 or set dimension = 3 within the "
            "parameter file"));
  }
}
...
int get_max_subsection_size(const std::string &file_name) {
  ...
  AssertThrow(
      ...
      dealii::ExcMessage(
          "Your parameter file does not contain any indication for the number "
          "of boundary conditions for any physics supported by Lethe. Since "
          "November 2023, Lethe requires that a \"boundary conditions\" "
          "subsection is present with at least \"number=0\" "));
...
void print_version_info(const ConditionalOStream &pcout) {
  ...
  pcout << std::endl;
#else
  AssertThrow(false, ExcMessage("To print version information using -V you "
                                "need a version of deal.II >= 9.7.0."));
#endif
}
...
parse_args(int argc, char **argv) {
  ...
  for (; argi < argc; ++argi) {
    ...
    auto n{arg.length()};
    if (n == 0 || arg[0] != '-' || n == 1) { // empty arg or single hyphen
      break;
    }
    if (arg[1] != '-') { // short option
      ...
      if (n == 2) { // done options on "--" and skip it
        argi++;
        break;
      }
      options[arg] = true;
...
#else
    AssertThrow(false, ExcMessage("To print only changed parameters you need a "
                                  "version of deal.II >= 9.7.0."));
#endif
...
void delete_vtu_and_pvd_files(const std::string &output_path) {
  ...
  for (auto const &filename :
       std::filesystem::directory_iterator{output_path}) {
...
#else
  AssertThrow(false, ExcMessage("Deleting old vtu files is not possible with "
