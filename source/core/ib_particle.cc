...
template <int dim> void IBParticle<dim>::initialize_all() {
...
template <int dim> void IBParticle<dim>::initialize_previous_solution() {
  // initialize all the variables associated to an immersed boundary particle
  previous_fluid_forces = fluid_forces;
  velocity_iter = velocity;
...
IBParticle<dim>::get_properties_name() {
  ...
  particle_properties = DEM::ParticleProperties<
      dim, DEM::CFDDEMProperties::PropertiesIndex>::get_properties_name();
...
template <int dim> std::vector<double> IBParticle<dim>::get_properties() {
...
template <int dim>
void IBParticle<dim>::closest_surface_point(
...
template <int dim>
void IBParticle<dim>::closest_surface_point(const Point<dim> &p,
                                            Point<dim> &closest_point) {
  shape->closest_surface_point(p, closest_point);
}

template <int dim>
bool IBParticle<dim>::is_inside_crown(
  ...
  if (absolute_distance) {
    ...
    is_outside_inner_ring = distance >= effective_radius * (inside_radius - 1);
  }
...
template <int dim>
bool IBParticle<dim>::is_inside_crown(const Point<dim> &evaluation_point,
...
template <int dim> void IBParticle<dim>::clear_shape_cache() {
  this->shape->clear_cache();
}

template <int dim>
void IBParticle<dim>::initialize_shape(
    const std::string &type, const std::vector<double> &shape_arguments) {
  shape = ShapeGenerator::initialize_shape_from_vector(type, shape_arguments,
                                                       position, orientation);
}

template <int dim>
void IBParticle<dim>::initialize_shape(const std::string &type,
                                       const std::string &raw_arguments) {
  shape = ShapeGenerator::initialize_shape(type, raw_arguments, position,
                                           orientation);
}

template <int dim>
void IBParticle<dim>::set_orientation(const Tensor<1, 3> &new_orientation) {
...
template <int dim>
void IBParticle<dim>::update_precalculations(
    DoFHandler<dim> &updated_dof_handler, const bool precalculations) {
...
template <int dim>
void IBParticle<dim>::remove_superfluous_data(
    DoFHandler<dim> &updated_dof_handler, const bool precalculations) {
  if (typeid(*shape) == typeid(RBFShape<dim>))
...
template <int dim> void IBParticle<dim>::load_data_from_file() {
  if (typeid(*shape) == typeid(RBFShape<dim>))
