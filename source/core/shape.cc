...
template <int dim> double Shape<dim>::displaced_volume() {
  StandardExceptions::ExcNotImplemented();
  return 0.;
}

template <int dim> void Shape<dim>::clear_cache() {
...
Shape<dim>::align_and_center(const Point<dim> &evaluation_point) const {
  ...
  if constexpr (dim == 2) {
    if (std::abs(theta[2]) > 1e-10 * this->effective_radius) {
      ...
      for (int j = 0; j < dim; ++j) {
        for (int k = 0; k < dim; ++k) {
          centralized_rotated[j] += rot_matrix[j][k] * centralized_point[k];
        }
      }
    }
  } else // (dim == 3)
...
template <int dim>
std::shared_ptr<Manifold<dim - 1, dim>> Shape<dim>::get_shape_manifold() {
...
template <int dim>
void Shape<dim>::closest_surface_point(
...
template <int dim>
void Shape<dim>::closest_surface_point(const Point<dim> &p,
...
template <int dim>
double Shape<dim>::value_with_cell_guess(
...
template <int dim>
Tensor<1, dim> Shape<dim>::gradient_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator /*cell*/,
    const unsigned int /*component*/) {
...
template <int dim>
double Sphere<dim>::value(const Point<dim> &evaluation_point,
                          const unsigned int /*component*/) const {
  return sphere_function->value(evaluation_point) - this->layer_thickening;
}

template <int dim>
std::shared_ptr<Shape<dim>> Sphere<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<Sphere<dim>>(
      this->effective_radius, this->position, this->orientation);
  return copy;
}

template <int dim>
Tensor<1, dim> Sphere<dim>::gradient(const Point<dim> &evaluation_point,
                                     const unsigned int /*component*/) const {
...
template <int dim>
std::shared_ptr<Manifold<dim - 1, dim>> Sphere<dim>::get_shape_manifold() {
  return std::make_shared<SphericalManifold<dim - 1, dim>>(this->position);
}

template <int dim> double Sphere<dim>::displaced_volume() {
  double solid_volume;
  using numbers::PI;
  if (dim == 2)
...
template <int dim> void Sphere<dim>::set_position(const Point<dim> &position) {
...
Sphere<dim>::distance_to_shape_with_cell_guess(
    Shape<dim> &shape,
    const typename DoFHandler<dim>::active_cell_iterator &cell,
    std::vector<Point<dim>> &candidate_points, double precision,
...
template <int dim>
std::tuple<double, Tensor<1, dim>, Point<dim>> Sphere<dim>::distance_to_shape(
    Shape<dim> &shape, std::vector<Point<dim>> &candidate_points,
    double precision, bool exact_distance_outside_of_contact) {
  (void)candidate_points;
  (void)precision;
  (void)exact_distance_outside_of_contact;
  double distance = DBL_MAX;
...
template <int dim> double Sphere<dim>::local_curvature_radius(Point<dim> p) {
  return (this->position - p).norm();
}

template <int dim>
double Sphere<dim>::local_curvature_radius_with_cell_guess(
    const Point<dim> p,
    const typename DoFHandler<dim>::active_cell_iterator cell) {
  (void)cell;
  return (this->position - p).norm();
...
template <int dim>
double Plane<dim>::value(const Point<dim> &evaluation_point,
                         const unsigned int /*component*/) const {
...
template <int dim> std::shared_ptr<Shape<dim>> Plane<dim>::static_copy() const {
...
template <int dim>
Tensor<1, dim> Plane<dim>::gradient(const Point<dim> &evaluation_point,
                                    const unsigned int /*component*/) const {
...
template <int dim> double Plane<dim>::displaced_volume() { return 0; }

template <int dim>
void Superquadric<dim>::closest_surface_point(
    const Point<dim> &p, Point<dim> &closest_point,
    const typename DoFHandler<dim>::active_cell_iterator & /*cell_guess*/) {
...
template <int dim>
void Superquadric<dim>::closest_surface_point(const Point<dim> &p,
                                              Point<dim> &closest_point) const {
  auto point_in_string = this->point_to_string(p);
  auto iterator = this->closest_point_cache.find(point_in_string);
  if (iterator == this->closest_point_cache.end()) {
...
template <int dim>
double Superquadric<dim>::value(const Point<dim> &evaluation_point,
                                const unsigned int /*component*/) const {
  using numbers::PI;
  ...
  if (iterator == this->value_cache.end()) {
    ...
    if (superquadric(centered_point) > 0)
      return (closest_point - evaluation_point).norm() - this->layer_thickening;
    else
...
template <int dim>
double Superquadric<dim>::value_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator /*cell*/,
...
template <int dim>
std::shared_ptr<Shape<dim>> Superquadric<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<Superquadric<dim>>(
      this->half_lengths, this->exponents, this->epsilon, this->position,
      this->orientation);
  return copy;
}
...
template <int dim>
Tensor<1, dim> Superquadric<dim>::gradient_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator /*cell*/,
    const unsigned int /*component*/) {
  auto point_in_string = this->point_to_string(evaluation_point);
...
template <int dim>
double OpenCascadeShape<dim>::value(const Point<dim> &evaluation_point,
                                    const unsigned int /*component*/) const {
#ifdef DEAL_II_WITH_OPENCASCADE
  auto point_in_string = this->point_to_string(evaluation_point);
  auto iterator = this->value_cache.find(point_in_string);
...
template <int dim>
double OpenCascadeShape<dim>::value_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator /*cell*/,
    const unsigned int /*component*/) {
#ifdef DEAL_II_WITH_OPENCASCADE
  ...
  if (iterator == this->value_cache.end()) {
    ...
    if (point_state == TopAbs_State::TopAbs_IN) {
      ...
      return (centered_point - projected_point).norm() - this->layer_thickening;
    }
  } else {
...
template <int dim>
std::shared_ptr<Shape<dim>> OpenCascadeShape<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<OpenCascadeShape<dim>>(
      local_file_name, this->position, this->orientation);
  return copy;
}
...
template <int dim>
Tensor<1, dim> OpenCascadeShape<dim>::gradient_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator /*cell*/,
    const unsigned int /*component*/) {
#ifdef DEAL_II_WITH_OPENCASCADE
...
template <int dim> double OpenCascadeShape<dim>::displaced_volume() {
  return 0;
}

template <int dim>
void OpenCascadeShape<dim>::set_position(const Point<dim> &position) {
  this->Shape<dim>::set_position(position);
}

template <int dim>
double HyperRectangle<dim>::value(const Point<dim> &evaluation_point,
                                  const unsigned int /*component*/) const {
  Point<dim> centered_point = this->align_and_center(evaluation_point);
...
template <int dim>
std::shared_ptr<Shape<dim>> HyperRectangle<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<HyperRectangle<dim>>(
      this->half_lengths, this->position, this->orientation);
  return copy;
}

template <int dim> double HyperRectangle<dim>::displaced_volume() {
...
template <int dim>
double Ellipsoid<dim>::value(const Point<dim> &evaluation_point,
                             const unsigned int /*component*/) const {
  Point<dim> centered_point = this->align_and_center(evaluation_point);
...
template <int dim>
std::shared_ptr<Shape<dim>> Ellipsoid<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<Ellipsoid<dim>>(
      this->radii, this->position, this->orientation);
  return copy;
}

template <int dim> double Ellipsoid<dim>::displaced_volume() {
  using numbers::PI;
...
template <int dim>
double Torus<dim>::value(const Point<dim> &evaluation_point,
                         const unsigned int /*component*/) const {
...
template <int dim> std::shared_ptr<Shape<dim>> Torus<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy =
      std::make_shared<Torus<dim>>(this->ring_radius, this->ring_thickness,
                                   this->position, this->orientation);
  return copy;
}

template <int dim> double Torus<dim>::displaced_volume() {
  using numbers::PI;
  return 2.0 * PI * PI * ring_radius * ring_thickness * ring_thickness;
}

template <int dim>
double Cone<dim>::value(const Point<dim> &evaluation_point,
                        const unsigned int /*component*/) const {
  AssertDimension(dim, 3);
...
template <int dim> std::shared_ptr<Shape<dim>> Cone<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<Cone<dim>>(
      this->tan_base_angle, this->height, this->position, this->orientation);
  return copy;
}

template <int dim> double Cone<dim>::displaced_volume() {
  using numbers::PI;
  return PI / 3.0 * base_radius * base_radius * height;
}

template <int dim>
double CutHollowSphere<dim>::value(const Point<dim> &evaluation_point,
                                   const unsigned int /*component*/) const {
  AssertDimension(dim, 3);
...
template <int dim>
std::shared_ptr<Shape<dim>> CutHollowSphere<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<CutHollowSphere<dim>>(
      this->radius, this->cut_depth, this->shell_thickness, this->position,
      this->orientation);
  return copy;
}

template <int dim> double CutHollowSphere<dim>::displaced_volume() {
  using numbers::PI;
...
template <int dim>
double DeathStar<dim>::value(const Point<dim> &evaluation_point,
                             const unsigned int /*component*/) const {
  AssertDimension(dim, 3);
...
template <int dim>
std::shared_ptr<Shape<dim>> DeathStar<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<DeathStar<dim>>(
      this->radius, this->hole_radius, this->spheres_distance, this->position,
      this->orientation);
  return copy;
}

template <int dim> double DeathStar<dim>::displaced_volume() {
  using numbers::PI;
  return 4. * PI / 3. * radius * radius * radius;
}

template <int dim>
double CompositeShape<dim>::value(const Point<dim> &evaluation_point,
                                  const unsigned int /*component*/) const {
  auto point_in_string = this->point_to_string(evaluation_point);
  auto iterator = this->value_cache.find(point_in_string);
  if (iterator == this->value_cache.end()) {
    ...
    double levelset;
    std::tie(levelset, std::ignore) = apply_boolean_operations(
        constituent_shapes_values, constituent_shapes_gradients);
    return levelset;
  } else
    return iterator->second;
}
...
template <int dim>
double CompositeShape<dim>::value_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator cell,
    const unsigned int /*component*/) {
  auto point_in_string = this->point_to_string(evaluation_point);
...
CompositeShape<dim>::gradient(const Point<dim> &evaluation_point,
  ...
  if (iterator == this->gradient_cache.end()) {
    ...
    double levelset;
    Tensor<1, dim> gradient;
    std::tie(levelset, gradient) = apply_boolean_operations(
        constituent_shapes_values, constituent_shapes_gradients);
...
template <int dim>
Tensor<1, dim> CompositeShape<dim>::gradient_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator cell,
    const unsigned int /*component*/) {
  auto point_in_string = this->point_to_string(evaluation_point);
...
template <int dim>
std::shared_ptr<Shape<dim>> CompositeShape<dim>::static_copy() const {
...
template <int dim>
void CompositeShape<dim>::update_precalculations(
...
template <int dim> void CompositeShape<dim>::load_data_from_file() {
  for (auto const &constituent : constituents | boost::adaptors::map_values)
    if (typeid(*constituent) == typeid(RBFShape<dim>))
      std::static_pointer_cast<RBFShape<dim>>(constituent)
          ->load_data_from_file();
...
template <int dim>
void CompositeShape<dim>::remove_superfluous_data(
    DoFHandler<dim> &updated_dof_handler,
    const bool mesh_based_precalculations) {
  for (auto const &constituent : constituents | boost::adaptors::map_values)
    if (typeid(*constituent) == typeid(RBFShape<dim>))
...
template <int dim>
std::pair<double, Tensor<1, dim>> CompositeShape<dim>::apply_boolean_operations(
    std::map<unsigned int, double> constituent_shapes_values,
  ...
  for (auto const &[operation_id, op_triplet] : operations) {
    ...
    case BooleanOperation::Union:
      if (value_first_component < value_second_component) {
        ...
        constituent_shapes_gradients[operation_id] = gradient_first_component;
      } else {
...
template <int dim> void CompositeShape<dim>::clear_cache() {
...
template <int dim>
void CompositeShape<dim>::set_layer_thickening(const double layer_thickening) {
...
RBFShape<dim>::RBFShape(const std::string &shape_arguments_str,
    ...
    : Shape<dim>(1, position, orientation), number_of_nodes(1),
      iterable_nodes(1), likely_nodes_map(), max_number_of_inside_nodes(1),
      maximal_support_radius(1), weights(1), nodes_positions(1),
      support_radii(1), basis_functions(1), useful_rbf_nodes(1) {
  this->filename = shape_arguments_str;
  this->load_data_from_file();
}

template <int dim>
double RBFShape<dim>::value_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator cell,
    const unsigned int /*component*/) {
...
template <int dim>
Tensor<1, dim> RBFShape<dim>::gradient_with_cell_guess(
    const Point<dim> &evaluation_point,
    const typename DoFHandler<dim>::active_cell_iterator cell,
    const unsigned int /*component*/) {
...
template <int dim>
double RBFShape<dim>::value(const Point<dim> &evaluation_point,
                            const unsigned int /*component*/) const {
  double bounding_box_distance = bounding_box->value(evaluation_point);
  if (bounding_box_distance >= 0)
    return bounding_box_distance + this->effective_radius -
...
template <int dim>
Tensor<1, dim> RBFShape<dim>::gradient(const Point<dim> &evaluation_point,
                                       const unsigned int /*component*/) const {
  Point<dim> centered_point = evaluation_point;
  double bounding_box_distance = bounding_box->value(centered_point);
  Tensor<1, dim> bounding_box_gradient = bounding_box->gradient(centered_point);
  ...
  if (iterable_nodes.size() == 0) {
    throw std::logic_error(
        "Every location inside the bounding box should be covered by the "
        "radius "
...
template <int dim>
std::shared_ptr<Shape<dim>> RBFShape<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<RBFShape<dim>>(
      this->filename, this->position, this->orientation);
  return copy;
}

template <int dim> double RBFShape<dim>::displaced_volume() {
  return bounding_box->displaced_volume();
}

template <int dim> void RBFShape<dim>::initialize_bounding_box() {
  ...
  bounding_box = std::make_shared<HyperRectangle<dim>>(
      half_lengths, this->reverse_align_and_center(bounding_box_center),
      this->orientation);
...
template <int dim>
void RBFShape<dim>::update_precalculations(
    DoFHandler<dim> &updated_dof_handler,
    const bool mesh_based_precalculations) {
  if (!mesh_based_precalculations)
    return;
  ...
  std::iota(std::begin(*temporary_node_numbers),
            std::end(*temporary_node_numbers), 0);
  ...
  for (unsigned int level = 0; level < maximal_level; level++) {
    ...
    for (const auto &cell : cell_iterator) {
      ...
      if (cell->level() > 0)
        try {
          ...
          if (parent_iterator != temporary_nodes_portions_map.end()) {
            temporary_node_numbers = std::get<2>(parent_iterator->second);
          } else
      ...
      if (!current_cell_nodes->empty())
        temporary_nodes_portions_map[cell] = std::make_tuple(
            cell->barycenter(), cell->diameter(), current_cell_nodes);
  ...
  for (const auto &cell : cell_iterator) {
    ...
    likely_nodes_map[cell] = std::make_shared<std::vector<std::tuple<
        Point<dim>, double, std::shared_ptr<std::vector<size_t>>>>>();
    for (auto it = temporary_nodes_portions_map.cbegin();
         it != temporary_nodes_portions_map.cend(); ++it) {
      temp_cell = it->first;
      if (temp_cell->is_active()) {
        temp_cell_tuple =
            std::make_tuple(temp_cell->barycenter(), temp_cell->diameter(),
                            std::get<2>(it->second));
...
template <int dim>
void RBFShape<dim>::determine_likely_nodes_for_one_cell(
...
template <int dim> void RBFShape<dim>::load_data_from_file() {
  weights.clear();
  support_radii.clear();
  ...
  iterable_nodes[0] = {Point<dim>{}, std::numeric_limits<double>::max(),
                       temp_nodes_id};
...
template <int dim>
void RBFShape<dim>::remove_superfluous_data(
    DoFHandler<dim> &updated_dof_handler,
    const bool mesh_based_precalculations) {
  if (!mesh_based_precalculations)
    return;
...
template <int dim> void RBFShape<dim>::rotate_nodes() {
  rotated_nodes_positions.clear();
...
template <int dim>
double RBFShape<dim>::evaluate_basis_function_derivative(
    const RBFBasisFunction basis_function, const double distance) const {
  switch (basis_function) {
  case RBFBasisFunction::WENDLANDC2:
...
template <int dim>
void RBFShape<dim>::swap_iterable_nodes(
    const typename DoFHandler<dim>::active_cell_iterator cell) {
...
template <int dim>
double Cylinder<dim>::value(const Point<dim> &evaluation_point,
                            const unsigned int /*component*/) const {
  Point<dim> centered_point = this->align_and_center(evaluation_point);
...
template <int dim>
double CylindricalTube<dim>::value(const Point<dim> &evaluation_point,
                                   const unsigned int /*component*/) const {
  Point<dim> centered_point = this->align_and_center(evaluation_point);
...
template <int dim>
std::shared_ptr<Shape<dim>> CylindricalTube<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<CylindricalTube<dim>>(
      this->radius + rectangular_base / 2, this->radius - rectangular_base / 2,
      this->height, this->position, this->orientation);
  return copy;
}

template <int dim> double CylindricalTube<dim>::displaced_volume() {
  using numbers::PI;
...
template <int dim>
double CylindricalHelix<dim>::value(const Point<dim> &evaluation_point,
                                    const unsigned int /*component*/) const {
  Point<dim> centered_point = this->align_and_center(evaluation_point);
...
template <int dim>
std::shared_ptr<Shape<dim>> CylindricalHelix<dim>::static_copy() const {
  std::shared_ptr<Shape<dim>> copy = std::make_shared<CylindricalHelix<dim>>(
      this->radius, this->radius_disk, this->height, this->pitch,
      this->position, this->orientation);
  return copy;
}

template <int dim> double CylindricalHelix<dim>::displaced_volume() {
  using numbers::PI;
