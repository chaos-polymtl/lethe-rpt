...
void Manifolds::declareDefaultEntry(ParameterHandler &prm,
  ...
  prm.declare_entry("cad file", "none", Patterns::FileName(), "IGES file name");
  prm.declare_entry(
      "point coordinates", "0,0,0", Patterns::List(Patterns::Double()),
      "Point coordinates describing the spherical or cylindrical manifold");
  prm.declare_entry("direction vector", "0,1,0",
                    Patterns::List(Patterns::Double()),
                    "Central axis describing the cylindrical manifold");
}

void Manifolds::parse_boundary(const ParameterHandler &prm,
                               const unsigned int i_bc) {
...
void Manifolds::declare_parameters(ParameterHandler &prm,
...
void Manifolds::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("manifolds");
  {
    ...
    if (manifolds.types[i] == Parameters::Manifolds::ManifoldType::spherical) {
      ...
      if constexpr (spacedim == 3) {
        ...
        Tensor<1, spacedim> cylinder_axis(
            value_string_to_tensor<spacedim>(manifolds.manifold_direction[i]));

        CylindricalManifold<dim, spacedim> manifold_description(cylinder_axis,
                                                                point_on_axis);
        triangulation.set_manifold(manifolds.id[i], manifold_description);
      } else
      ...
      attach_cad_to_manifold(triangulation, manifolds.cad_files[i],
                             manifolds.id[i]);
    } else if (manifolds.types[i] ==
