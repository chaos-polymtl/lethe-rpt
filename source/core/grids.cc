...

{
      ...
      auto construction_data = TriangulationDescription::Utilities::
          create_description_from_triangulation_in_groups<dim, spacedim>(
              ...
              [](dealii::Triangulation<dim, spacedim> &basetria,
                 const MPI_Comm comm, const unsigned int /*group_size*/) {
                ...
                AssertThrow(
                    false,
                    ExcMessage(
                        "Parallel simulation with simplex meshes require that "
                        "deal.II be compiled with either Metis or Zoltan"));
#endif
              },
              comm, Utilities::MPI::n_mpi_processes(comm) /* group size */,
  ...
  // Dealii grids
    ...
    if (mesh_parameters.simplex) {
      ...
      GridGenerator::generate_from_name_and_arguments(
          temporary_quad_triangulation, mesh_parameters.grid_type,
      ...
      auto construction_data = TriangulationDescription::Utilities::
          create_description_from_triangulation(
              temporary_tri_triangulation, triangulation.get_mpi_communicator(),
              TriangulationDescription::Settings::
                  construct_multigrid_hierarchy);
      triangulation.create_triangulation(construction_data);
    } else {
      GridGenerator::generate_from_name_and_arguments(
          triangulation, mesh_parameters.grid_type,
          mesh_parameters.grid_arguments);
      ...
      if constexpr (dim == 3) {
        ...
        GridTools::rotate(mesh_parameters.rotation_axis,
                          mesh_parameters.rotation_angle, triangulation);
      }
    }
  }
  // Customizable cylinder mesh
    ...
    if (mesh_parameters.simplex) {
      throw std::runtime_error(
          "Unsupported mesh type - custom cylinder mesh with simplex is not "
          "supported. Use a dealii cylinder to use simplex mesh.");
    } else if (dim != 3) {
      throw std::runtime_error("Unsupported mesh type - custom cylinder mesh "
                               "is only supported in 3d.");
    }
    ...
    if constexpr (dim == 3) {
      ...
      if (arguments.size() != 3) {
        throw std::runtime_error("Mandatory cylinder parameters are (x "
                                 "subdivisions: radius : half height)");
      } else {
      ...
      if (mesh_parameters.grid_type == "classic") {
        ...
        GridGenerator::subdivided_cylinder(triangulation, subdivisions, radius,
                                           half_height);
        ...
        if (mesh_parameters.grid_type == "regularized" ||
          ...
          GridGenerator::hyper_cube(temporary_triangulation, -real_radius,
                                    real_radius, true);
          ...
          GridGenerator::hyper_ball_balanced(temporary_triangulation, center,
                                             radius);
        } else {
          throw std::runtime_error("Unknown grid type. Choices are "
                                   "<classic|balanced|squared|regularized>.");
        }
        ...
        GridGenerator::extrude_triangulation(
            temporary_triangulation, subdivisions + 1, 2.0 * half_height,
            triangulation, true);
  ...
  // Periodic Hills grid
    ...
    if (mesh_parameters.simplex) {
      throw std::runtime_error("Unsupported mesh type - periodic hills mesh "
                               "with simplex is not supported");
    } else {
  ...
  for (auto const &[id, type] : boundary_conditions.type) {
    if (type == BoundaryConditions::BoundaryType::periodic) {
      ...
      GridTools::collect_periodic_faces(
          *dynamic_cast<Triangulation<dim, spacedim> *>(triangulation_ptr), id,
          boundary_conditions.periodic_neighbor_id.at(id),
          boundary_conditions.periodic_direction.at(id), periodicity_vector);
...
void read_mesh_and_manifolds(
    parallel::DistributedTriangulationBase<dim, spacedim> &triangulation,
    const Parameters::Mesh &mesh_parameters,
    const Parameters::Manifolds &manifolds_parameters, const bool restart,
    const BoundaryConditions::BoundaryConditions &boundary_conditions) {
  ...
  if (mesh_parameters.type == Parameters::Mesh::Type::gmsh) {
    ...
    if (manifolds_parameters.size > 0) {
      for (const auto &face : triangulation.active_face_iterators()) {
        if (face->at_boundary() && manifold_ids.contains(face->boundary_id()))
  ...
  if (mesh_parameters.simplex) {
    ...
    if (mesh_parameters.refine_until_target_size) {
      ...
      unsigned int number_refinement = static_cast<unsigned int>(
          floor(std::log(minimal_cell_size / target_size) / std::numbers::ln2));
      ...
      refine_triangulation_at_boundaries(
          ...
          mesh_parameters.initial_refinement_at_boundaries, triangulation);
    }
  }
}
...
void read_mesh_and_manifolds_for_stator_and_rotor(
  ...
  AssertThrow(mesh_parameters.type == mortar_parameters.rotor_mesh->type,
              ExcMessage("The mesh types for the rotor and stator geometries "
                         "must be the same."));
  ...
  if (mesh_parameters.type == Parameters::Mesh::Type::dealii) {
    ...
    GridGenerator::merge_triangulations(stator_temp_tria, rotor_temp_tria,
                                        triangulation, 0.0, true, true);
    ...
    for (unsigned int j = stator_ids_no_flat;
         j < stator_ids_no_flat + rotor_ids_no_flat; j++) {
  ...
  AssertThrow(n_faces_rotor_interface == n_faces_stator_interface,
              ExcMessage("The number of faces at the rotor interface ID #" +
                         ...
                         " is different from the number of faces at the stator "
                         "interface ID #" +
...
template void read_mesh_and_manifolds_for_stator_and_rotor(
    parallel::DistributedTriangulationBase<2> &triangulation,
    const Parameters::Mesh &mesh_parameters,
    const Parameters::Manifolds &manifolds_parameters, const bool restart,
    const BoundaryConditions::BoundaryConditions &boundary_conditions,
