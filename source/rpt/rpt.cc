...
template <int dim> void RPT<dim>::setup_and_calculate() {
  ...
  if (rpt_parameters.tuning_param.tuning) {
    ...
    AssertThrow(
        ...
        ExcMessage(
            ...
            "Note : The experimental counts also have to be at the same "
            "positions which can not be verified."));
...
template <int dim> void RPT<dim>::calculate_counts() {
  ...
  for (unsigned int i_particle = 0; i_particle < particle_positions.size();
    ...
    for (unsigned int i_detector = 0; i_detector < detectors.size();
      ...
      ParticleDetectorInteractions<dim> particle_detector_interactions(
          particle_positions[i_particle], detectors[i_detector],
...
template <int dim> void RPT<dim>::export_data() {
  // Open a file
  std::ofstream myfile;
  std::string sep;
  ...
  if (filename.substr(filename.find_last_of('.') + 1) == ".dat") {
    myfile << "particle_positions_x particle_positions_y particle_positions_z "
              "detector_id counts"
           << std::endl;
    sep = " ";
  } else // .csv is default
  ...
  for (unsigned int i_particle = 0; i_particle < particle_positions.size();
    ...
    for (unsigned int i_detector = 0; i_detector < detectors.size();
         i_detector++) {
      myfile << particle_positions[i_particle].get_position()[0] << sep
