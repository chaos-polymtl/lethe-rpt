...
template <int dim>
void RPTCellReconstruction<dim>::execute_cell_reconstruction() {
  ...
  if (reconstruction_parameters.analyse_positions) {
    ...
    AssertThrow(known_positions.size() == reconstruction_counts.size(),
                ExcMessage("Reconstruction counts and known positions files do "
                           "not have data for the same number of positions."));
  }
...
template <int dim> void RPTCellReconstruction<dim>::set_coarse_mesh_counts() {
  find_vertices_positions(reconstruction_parameters.coarse_mesh_level);
  calculate_counts();
}

template <int dim> void RPTCellReconstruction<dim>::find_all_positions() {
...
template <int dim>
void RPTCellReconstruction<dim>::find_unknown_position(
    std::vector<double> &particle_reconstruction_counts, unsigned int id) {
  int level = 0;
...
template <int dim>
void RPTCellReconstruction<dim>::find_vertices_positions(
    unsigned int level, std::vector<int> parent_cell_indexes /* {-1} */) {
  ...
  for (const auto &cell : triangulation.cell_iterators_on_level(level)) {
    ...
    if (parent_is_candidate) {
      ...
      for (unsigned int i = 0; i < cell->n_vertices(); i++) {
        ...
        if (map_vertices_index.find(vertex_index) == map_vertices_index.end()) {
...
template <int dim> void RPTCellReconstruction<dim>::calculate_counts() {
...
template <int dim>
std::vector<int> RPTCellReconstruction<dim>::find_cells(
    unsigned int level, std::vector<double> &particle_reconstruction_counts,
    ...
    if (parent_is_candidate) {
      for (unsigned int j = 0; j < detectors.size(); j++) {
        ...
        for (unsigned int i = 0; i < cell->n_vertices(); i++) {
          ...
          unsigned int vertex_index = cell->vertex_index(i);
          counts_vertices[i] = map_vertices_index[vertex_index].second[j];
        }
      ...
      // cell in candidate
      if (std::all_of(candidate_detector.begin(), candidate_detector.end(),
                      [](bool v) { return v; }))
...
template <int dim>
int RPTCellReconstruction<dim>::find_best_cell(
    unsigned int level, std::vector<double> &particle_reconstruction_counts,
    ...
    if (is_candidate) {
      ...
      for (unsigned int j = 0; j < detectors.size(); j++) {
        for (unsigned int i = 0; i < cell->n_vertices(); i++) {
          unsigned int vertex_index = cell->vertex_index(i);
          error +=
              std::pow(std::fabs(particle_reconstruction_counts[j] -
  ...
  std::sort(cellid_error.begin(), cellid_error.end(),
...
template <int dim> void RPTCellReconstruction<dim>::export_positions() {
  std::ofstream myfile;
  std::string sep;
  std::string filename =
  ...
  if (filename.substr(filename.find_last_of('.') + 1) == ".dat") {
    myfile << "reconstructed_positions_id volumes particle_positions_x "
              "particle_positions_y particle_positions_z cell_level";
    sep = " ";
  } else // .csv is default
  {
    myfile << "reconstructed_positions_id,volumes,particle_positions_x,"
  ...
  for (unsigned int i_particle = 0;
       i_particle < reconstruction_positions.size(); i_particle++) {
...
template <int dim> void RPTCellReconstruction<dim>::visualize_positions() {
  TimerOutput::Scope t(computing_timer, "visualization_files_creation");

  ParticleVisualization<dim> particle_visualization(
      triangulation, reconstruction_parameters.reconstruction_positions_file,
      reconstruction_positions, reconstruction_counts);
