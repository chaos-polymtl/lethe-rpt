...
void Parameters::RPTParameters::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("rpt parameters");
  {
    prm.declare_entry("particle positions file", "none", Patterns::FileName(),
    ...
    prm.declare_entry(
        "export counts", "false", Patterns::Bool(),
        "Enable to export counts result in a .csv file <true|false>");

    prm.declare_entry("counts file", "counts.csv", Patterns::FileName(),
                      "Exported count's results filename");

    prm.declare_entry("monte carlo iteration", "1", Patterns::Integer(),
                      "Number of Monte Carlo iteration");

    prm.declare_entry("random number seed", "auto", Patterns::Anything(),
                      "Seed of the random number generator <auto|number>");

    prm.declare_entry("reactor height", "0.1", Patterns::Double(),
                      "Height of the reactor or tank");

    prm.declare_entry("reactor radius", "0.1", Patterns::Double(),
                      "Radius of the reactor or tank");

    prm.declare_entry("peak-to-total ratio", "1", Patterns::Double(),
                      "Peak-to-total ratio");

    prm.declare_entry("sampling time", "1", Patterns::Double(),
                      "Sampling time of the counting");

    prm.declare_entry("gamma-rays emitted", "1", Patterns::Double(),
                      "Number of gamma-rays emitted by each disintegration");

    prm.declare_entry("attenuation coefficient detector", "1",
                      Patterns::Double(),
...
void Parameters::RPTParameters::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("rpt parameters");
  {
...
void Parameters::RPTTuningParameters::declare_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("parameter tuning");
  {
    prm.declare_entry("tuning", "false", Patterns::Bool(),
                      "Enable parameter tuning <true|false>");

    prm.declare_entry("cost function type", "larachi",
                      Patterns::Selection("larachi|l1|l2"),
                      "Cost function used for the parameter tuning"
                      "Choices are <larachi|l1|l2>.");

    prm.declare_entry("experimental data file", "none", Patterns::FileName(),
                      "Experimental counts data file name");
  }
  prm.leave_subsection();
}

void Parameters::RPTTuningParameters::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("parameter tuning");
  {
    ...
    if (type == "larachi")
      ...
      throw std::logic_error("Error, invalid cost function type. Choices are "
                             "'larachi', 'l2' or 'l1'.");
...
void Parameters::DetectorParameters::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("detector parameters");
  {
    ...
    prm.declare_entry("detector positions file", "none", Patterns::FileName(),
                      "Detector positions file name");

    prm.declare_entry("dead time", "1", Patterns::List(Patterns::Double()),
                      "Dead time of the detector per accepted pulse");

    prm.declare_entry("activity", "1", Patterns::List(Patterns::Double()),
                      "Activity of the tracer");

    prm.declare_entry("attenuation coefficient reactor", "1",
...
void Parameters::DetectorParameters::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("detector parameters");
  {
...
void Parameters::RPTReconstructionParameters::declare_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("reconstruction");
  {
    prm.declare_entry("refinement", "1", Patterns::Integer(),
                      "Number of refinement for the reactor");

    prm.declare_entry(
        "coarse mesh level", "0", Patterns::Integer(),
        "Level of the coarse mesh where all the counts of the first vertices");

    prm.declare_entry(
        "reconstruction counts file", "reconstruction_counts",
        Patterns::FileName(),
    ...
    prm.declare_entry("export reconstruction positions file",
                      "reconstruction_positions", Patterns::FileName(),
                      "Export positions found by reconstruction");

    prm.declare_entry(
        "analyse positions", "false", Patterns::Bool(),
        "Enable evaluation of results with known positions <true|false>");

    prm.declare_entry("known positions file", "known_positions_file",
                      Patterns::FileName(),
                      "Known positions (artificial or from experimental");

    prm.declare_entry("verbose clock", "false", Patterns::Bool(),
                      "Allow to show total wallclock time elapsed since start");
  }
  prm.leave_subsection();
}

void Parameters::RPTReconstructionParameters::parse_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("reconstruction");
  {
    ...
    if (coarse_mesh_level > reactor_refinement)
      throw std::logic_error("Error, the level of the coarse mesh can't be "
                             "higher that the refinement value (max level).");
  }
  prm.leave_subsection();
}

void Parameters::RPTFEMReconstructionParameters::declare_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("fem reconstruction");
  {
    prm.declare_entry("mesh type", "dealii", Patterns::Selection("dealii|gmsh"),
                      "Type of mesh used");

    prm.declare_entry("mesh filename", "none", Patterns::FileName(),
                      "Imported mesh filename");

    prm.declare_entry("z subdivisions", "1", Patterns::Integer(),
                      "Number of subdivisions of the initial grid in z");

    prm.declare_entry("mesh refinement", "0", Patterns::Integer(),
                      "Number of refinements the grid undergoes");

    prm.declare_entry(
        "l2 projection before reconstruction", "false", Patterns::Bool(),
        "Run the rpt_l2_projection_3 application before the reconstruction");

    prm.declare_entry("search extrapolation limit", "0.005", Patterns::Double(),
                      "Tolerance when extrapolating from a cell in the "
                      "reference space to find the particle's position");

    prm.declare_entry("experimental counts file", "none", Patterns::FileName(),
                      "Experimental counts filename");

    prm.declare_entry("export positions file", "none", Patterns::FileName(),
                      "Exported particle positions filename");

    prm.declare_entry("cost function type", "relative",
                      Patterns::Selection("absolute|relative"),
                      "Type of cost function applied when evaluating the "
                      "particle's real position");

    prm.declare_entry("dof handler file", "none", Patterns::FileName(),
                      "Saved DOF Handler filename");

    prm.declare_entry("nodal counts file", "none",
                      Patterns::List(Patterns::FileName()),
                      "Saved nodal counts of every detector for every cell of "
                      "the mesh filename");

    prm.declare_entry("search type", "local",
                      Patterns::Selection("local|global"),
                      "Type of search algorithm used when finding the "
                      "particle's real position");

    prm.declare_entry("search cell proximity level", "1", Patterns::Integer(),
                      "Level of proximity of cells considered in the search of "
                      "a position from the previous position");
...
void Parameters::RPTFEMReconstructionParameters::parse_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("fem reconstruction");
  {
