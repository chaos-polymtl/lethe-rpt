...
template <int dim> void RPTL2Projection<dim>::setup_triangulation() {
  if (fem_reconstruction_parameters.mesh_type ==
    ...
    GridGenerator::subdivided_cylinder(
        temp_triangulation, fem_reconstruction_parameters.z_subdivisions,
        parameters.reactor_radius, parameters.reactor_height * 0.5);
...
template <int dim> void RPTFEMReconstruction<dim>::setup_triangulation() {
...
template <int dim> void RPTL2Projection<dim>::setup_system() {
...
template <int dim>
void RPTL2Projection<dim>::solve_linear_system(unsigned detector_no) {
...
template <int dim>
void RPTL2Projection<dim>::assemble_system(unsigned no_detector) {
    ...
    if (cell->subdomain_id() ==
      ...
      for (const unsigned int q_index : fe_values.quadrature_point_indices()) {
        ...
        if (count < 0 || count != count)
          this->pcout << "Invalid value encountered in count calculation, this "
...
template <int dim> void RPTL2Projection<dim>::output_results() {
...
template <int dim> void RPTL2Projection<dim>::output_raw_results() {
  ...
  if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0) {
    ...
    if (parameters.verbosity == Parameters::Verbosity::verbose) {
      for (auto it = dof_index_and_location.begin();
           it != dof_index_and_location.end(); ++it) {
        for (int p = 0; p < dim; ++p)
...
template <int dim> void RPTL2Projection<dim>::checkpoint() {
...
template <int dim> void RPTL2Projection<dim>::L2_project() {
  pcout << "***********************************************" << std::endl;
  ...
  if (cost_function_type ==
    ...
    for (int i = 0; i < dim; ++i) {
      for (int j = 0; j < dim; ++j) {
        for (unsigned int d = 0; d < detector_size; ++d) {
          sigma += (-vertex_count[d][0] + vertex_count[d][j + 1]) *
                   (-vertex_count[d][0] + vertex_count[d][i + 1]) * denom[d];
        }
        sys_matrix.set(i, j, sigma);
        sigma = 0;
      }
...
template <int dim>
double RPTFEMReconstruction<dim>::calculate_reference_location_error(
...
template <int dim>
double RPTFEMReconstruction<dim>::calculate_cost(
...
template <int dim>
void RPTFEMReconstruction<dim>::search_position_in_reference_space(
  ...
  reference_location = assemble_matrix_and_rhs<dim>(
      count_from_all_detectors, experimental_count,
  ...
  norm_error_coordinates = calculate_reference_location_error(
      reference_location, last_constraint_reference_location);

  // Extrapolation limit
  if (norm_error_coordinates < tol_reference_location) {
    // Calculate cost with the selected cost function
    calculated_cost =
        calculate_cost(cell, reference_location,
                       last_constraint_reference_location, experimental_count);
    ...
    if (calculated_cost < max_cost) {
      ...
      for (unsigned int v = 0; v < cell->n_vertices(); ++v) {
        if (v == 0)
          ...
          real_location +=
              reference_location[v - 1] * (cell->vertex(v) - cell->vertex(0));
      }
    }
  }
}
...
template <int dim>
bool RPTFEMReconstruction<dim>::find_position_global_search(
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) {
    ...
    search_position_in_reference_space(
        count_from_all_detectors, experimental_counts, cell,
        tol_reference_location, max_cost, reference_location, position_found,
        real_location);
  }
...
template <int dim>
bool RPTFEMReconstruction<dim>::find_position_local_search(
    std::vector<double> &experimental_counts,
    const double tol_reference_location) {
  double max_cost = DBL_MAX;
  bool position_found = false;
    ...
    for (unsigned int proximity_level = 2;
      ...
      for (const auto &current_main_cell : previous_adjacent_cells) {
        if (*previous_main_cells.find(current_main_cell) != current_main_cell) {
          for (unsigned int v = 0; v < current_main_cell->n_vertices(); ++v) {
  ...
  for (const auto &adjacent_cell : all_adjacent_cells) {
    ...
    search_position_in_reference_space(
        count_from_all_detectors, experimental_counts, adjacent_cell,
        tol_reference_location, max_cost, reference_location, position_found,
        real_location);
  }
...
template <int dim> void RPTFEMReconstruction<dim>::trajectory() {
    ...
    if (fem_reconstruction_parameters.search_type ==
      ...
      for (std::vector<double> &experimental_counts : all_experimental_counts) {
        if (adjacent_cell_search)
          adjacent_cell_search = find_position_local_search(
              experimental_counts, tol_reference_location);
        else
          adjacent_cell_search = find_position_global_search(
              experimental_counts, tol_reference_location);
      }
    } else {
      // Find the position of the particle with the experimental counts by
...
template <int dim> void RPTFEMReconstruction<dim>::load_from_checkpoint() {
...
template <int dim> void RPTFEMReconstruction<dim>::export_found_positions() {
...
template <int dim> void RPTFEMReconstruction<dim>::rpt_fem_reconstruct() {
  std::cout << "***********************************************" << std::endl;
