...
ParticleVisualization<dim>::ParticleVisualization(
    Triangulation<dim> &background_triangulation, std::string &filename,
    std::vector<Point<dim>> &positions,
    std::vector<std::vector<double>> &counts)
    : particle_positions(positions), particle_counts(counts),
      visualization_filename(filename), mapping(1) {
...
template <int dim>
void ParticleVisualization<dim>::build_visualization_files() {
  ...
  write_boundaries_vtu<dim>(grid_out_faces, "./", 0, 0, MPI_COMM_WORLD,
                            filename, 0);
...
template <int dim>
void ParticleVisualization<dim>::output_particles(unsigned int it) {
  ...
  particle_output.build_patches(particle_handler, detector_ids,
                                data_component_interpretation);

  std::string s = visualization_filename;
  std::string filename = s.substr(0, s.find('.')) + "_";
  particle_output.write_vtu_with_pvtu_record("./", filename, it,
                                             MPI_COMM_WORLD);
}
