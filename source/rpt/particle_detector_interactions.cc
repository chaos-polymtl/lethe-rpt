...
template <int dim>
void ParticleDetectorInteractions<dim>::calculate_position_parameters() {
...
template <int dim>
void ParticleDetectorInteractions<dim>::calculate_solid_angle(double n_alpha,
                                                              double n_theta) {
  double alpha_max;
  ...
  if (rho > detector_radius) {
           ...
           // not usual
    {
      AssertThrow(
          h >= 0,
          ExcMessage("For now, it is not possible to calculate count for "
                     "particle positions on the side of the detector."));
    }
...
template <int dim>
double ParticleDetectorInteractions<dim>::calculate_detector_path_length() {
...
std::vector<double> ParticleDetectorInteractions<dim>::solve_t(
    Tensor<2, dim> e_inverse, Tensor<1, dim> detector_particle_origin,
  ...
  auto F = [=, *this](const double t) {
    ...
    return std::pow(
               (e_inverse * (line_equations) + detector_particle_origin)[0],
               2) +
           std::pow(
               (e_inverse * (line_equations) + detector_particle_origin)[1],
               2) -
           std::pow(R, 2);
  };
...
template <int dim>
double ParticleDetectorInteractions<dim>::calculate_reactor_path_length() {
  ...
  std::vector<double> t =
      solve_t(invert(detector_orientation_matrix), detector_particle_origin,
              particle_position_rotation);
...
template <int dim>
double ParticleDetectorInteractions<dim>::calculate_non_interaction_probability(
    double &reactor_path_length) {
...
template <int dim>
void ParticleDetectorInteractions<dim>::calculate_efficiency() {
...
template <int dim> double ParticleDetectorInteractions<dim>::calculate_count() {
...
template <int dim> double ParticleDetectorInteractions<dim>::get_h() {
  calculate_position_parameters();

  return h;
}

template <int dim> double ParticleDetectorInteractions<dim>::get_rho() {
  calculate_position_parameters();
...
template <int dim>
double ParticleDetectorInteractions<dim>::get_alpha(double n_alpha,
                                                    double n_theta) {
  calculate_position_parameters();
  calculate_solid_angle(n_alpha, n_theta);
...
template <int dim>
double ParticleDetectorInteractions<dim>::get_theta(double n_alpha,
                                                    double n_theta) {
  calculate_position_parameters();
  calculate_solid_angle(n_alpha, n_theta);
