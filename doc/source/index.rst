...
`Lethe repository <https://github.com/chaos-polymtl/lethe>`_ up to
Lethe v1.0.3. Since Lethe is meant to be mostly a single and
multiphase computational fluid dynamics software framework,
it was deemed necessary to move  the RPT modules of to a separate
repository since they are not in active development anymore.
However, they remain fully functional and are still supported
by the `CHAOS laboratory <https://www.chaos-lab.ca/>`_. 

Lethe is based on `deal.II <https://www.dealii.org/>`_, a finite element library.
