...
template <int dim> class Detector {
public:
  /**
   ...
   *
   */
  Detector(Parameters::DetectorParameters &detector_parameters, int n,
           Point<dim> &face_point, Point<dim> &middle_point)
      : radius(detector_parameters.radius), length(detector_parameters.length),
        id(n), face_position(face_point), middle_position(middle_point),
  ...
  Point<dim> get_face_position() { return face_position; }

  Point<dim> get_middle_position() { return middle_position; }

  double get_dead_time() { return dead_time; }

  double get_activity() { return activity; }

  double get_attenuation_coefficient_reactor() {
