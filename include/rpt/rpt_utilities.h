 ...
 */
template <int dim>
void attach_grid_to_triangulation(Triangulation<dim> &triangulation,
  ...
  // with z towards top)
  GridGenerator::subdivided_cylinder(triangulation, 5,
 ...
 */
template <int dim>
std::vector<Point<dim>> read_positions(std::string &positions_filename) {
  ...
  for (unsigned int i = 0; i < number_of_positions; i++) {
    Point<dim> point(values[dim * i], values[dim * i + 1], values[dim * i + 2]);
...
assign_detector_positions(Parameters::DetectorParameters &detector_parameters) {
  ...
  for (int i = 0; i < number_of_detector; i++) {
    Point<dim> face_point(values[2 * dim * i], values[2 * dim * i + 1],
                          values[2 * dim * i + 2]);
 ...
 */
template <int dim>
std::vector<double> read_counts(std::string &counts_filename) {
  ...
  std::copy(std::istream_iterator<double>(counts_file),
            std::istream_iterator<double>(), std::back_inserter(counts));
...
read_detectors_counts(std::string &counts_filename, unsigned int n_detectors) {
  ...
  for (unsigned int i = 0; i < counts.size(); i += n_detectors) {
    ...
    std::vector<double>::const_iterator last = counts.begin() + i + n_detectors;
