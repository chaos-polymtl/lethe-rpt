...
enum ThermalBoundaryType { adiabatic, isothermal };

template <int dim> class NitscheObject {
public:
  NitscheObject() : solid_velocity(dim) {}

  void declare_parameters(ParameterHandler &prm, unsigned int id);
  void parse_parameters(ParameterHandler &prm, unsigned int id);
...
template <int dim>
void NitscheObject<dim>::declare_parameters(ParameterHandler &prm,
                                            unsigned int id) {
  prm.enter_subsection("nitsche solid " + Utilities::int_to_string(id, 1));
  {
    ...
    prm.declare_entry("enable particles motion", "false", Patterns::Bool(),
                      "Condition on the motion of particles");
    prm.declare_entry("enable heat boundary condition", "false",
                      Patterns::Bool(),
                      "controls if a heat boundary condition is imposed on the "
                      "Nitsche immersed boundary");

    prm.declare_entry("beta", "10", Patterns::Double(),
                      "Penalization term for Nitsche method");
    prm.declare_entry(
        "beta heat", "10", Patterns::Double(),
        "Penalization term for Nitsche method applied to the heat equation");

    prm.declare_entry("particles sub iterations", "1", Patterns::Integer(),
                      "Number of sub iterations for the motion of the "
                      "particles. This parameter"
                      "enables the uses of a higher CFL condition for the "
                      "Nitsche solver while preventing the loss of particles");

    prm.declare_entry(
        "stop if particles lost", "true", Patterns::Bool(),
        "Enable stopping the simulation if Nitsche particles have been lost");

    prm.declare_entry(
        "number quadrature points", "2", Patterns::Integer(),
        "Number of Nitsche (quadrature) points to insert in a 1D cell. The "
        "number of"
    ...
    if constexpr (dim == 2) {
      prm.declare_entry("center of rotation", "0., 0.",
                        Patterns::List(Patterns::Double()),
                        "Solid object center of rotation");
    }
    if constexpr (dim == 3) {
      prm.declare_entry("center of rotation", "0., 0., 0.",
                        Patterns::List(Patterns::Double()),
                        "Solid object center of rotation");
    }

    prm.declare_entry("calculate force on solid", "false", Patterns::Bool(),
                      "Enable calculation of forces on solid");
    prm.declare_entry("calculate torque on solid", "false", Patterns::Bool(),
                      "Enable calculation of torques on solid");
    prm.declare_entry("solid force name", "force_solid", Patterns::FileName(),
                      "File output solid force prefix");
    prm.declare_entry("solid torque name", "torque_solid", Patterns::FileName(),
...
template <int dim>
void NitscheObject<dim>::parse_parameters(ParameterHandler &prm,
                                          unsigned int id) {
  prm.enter_subsection("nitsche solid " + Utilities::int_to_string(id, 1));
  {
...
template <int dim> class Nitsche {
public:
  Nitsche() {}
...
template <int dim>
void Nitsche<dim>::declare_parameters(ParameterHandler &prm) {
...
template <int dim> void Nitsche<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("nitsche");
  {
...
template <int dim> class RigidSolidObject {
public:
  RigidSolidObject() {}
...
template <int dim>
void RigidSolidObject<dim>::declare_parameters(ParameterHandler &prm,
                                               unsigned int id) {
  ...
  prm.enter_subsection("solid object " + Utilities::int_to_string(id, 1));
  {
    ...
    prm.declare_entry("thermal boundary type", "adiabatic",
    ...
    prm.declare_entry("output solid object", "true", Patterns::Bool(),
...
template <int dim>
void RigidSolidObject<dim>::parse_parameters(ParameterHandler &prm,
                                             unsigned int id) {
  // Use ParsedFunction<dim> during parameter declaration. We need to do this
...
template <int dim> class DEMSolidObjects {
public:
  DEMSolidObjects() {}
...
template <int dim>
void DEMSolidObjects<dim>::declare_parameters(ParameterHandler &prm) {
...
template <int dim>
void DEMSolidObjects<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("solid objects");
  {
    prm.enter_subsection("solid surfaces");
    {
