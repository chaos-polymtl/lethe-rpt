...
/**
 * @brief Non-linear Newton solver for non-linear systems of equations
 * implemented in the SUNDIALS suite, specifically the KINSOL package. This
 * solver has internal algorithms to determine the time step and decide whether
 * to reassemble the Jacobian matrix or not.
 */
...
KinsolNewtonNonLinearSolverStrategy<VectorType>::
    KinsolNewtonNonLinearSolverStrategy(
        PhysicsSolver<VectorType> *physics_solver,
        const Parameters::NonLinearSolver &params)
    : PhysicsSolverStrategy<VectorType>(physics_solver, params) {}

template <typename VectorType>
void KinsolNewtonNonLinearSolverStrategy<VectorType>::solve() {
...
#else
  throw std::runtime_error("Kinsol newton nonlinear solver requires DEAL_II to "
                           "be compiled with SUNDIALS");
