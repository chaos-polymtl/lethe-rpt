...
/**
 * @brief Abstract class that defines the interface for a interface property
 * model. Interface property model provides an abstract interface to calculate
 * the value of an interface property or a vector of interface property value
 * for given field value. By default, the interface does not require that all
 * (or any) fields be specified. This is why a map is used to pass the dependent
...
public:
  /**
   * @brief class Default constructor. Set the model_depends_on to false for all
   * variables.
   */
  ...
  /**
   * @brief Returns true if the InterfacePropertyModel depends on a field, false
   * if not.
   */
  ...
  /**
   * @brief vector_value Calculates the values of an interface property for
   * multiple points
   * @param field_vectors
   */
  virtual void
   ...
   * interface property with respect to a field
