 ...
 */
template <int dim> class ALE {
public:
  /**
   * @brief Construct a new ALE object.
   */
  ALE() { velocity = std::make_shared<Functions::ParsedFunction<dim>>(dim); }
...
template <int dim> void ALE<dim>::declare_parameters(ParameterHandler &prm) {
...
template <int dim> void ALE<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("ALE");
