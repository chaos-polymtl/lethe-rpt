...
public:
  ...
  /**
   * @brief Constructor for the linear physics. Since the Physics is linear, a
   * LinearSolutionStrategy is automatically generated as a
   ...
   *
   */
  virtual ~PhysicsSolver() { delete physics_solving_strategy; }
   ...
   */
  virtual void assemble_system_rhs() = 0;

  /**
   * @brief Set up the preconditioner.
   *
   */
  virtual void setup_preconditioner() = 0;
  ...
  /**
   * @brief Solve the global system of equations according to a given strategy,
   * either linear or not.
   */
  void solve_governing_system();
   ...
   */
  virtual VectorType &get_evaluation_point() = 0;
  virtual VectorType &get_local_evaluation_point() = 0;
  virtual VectorType &get_newton_update() = 0;
  virtual VectorType &get_present_solution() = 0;
  virtual VectorType &get_system_rhs() = 0;
  ...
  /**
   * @brief Return the metric for residual rescaling. By default, should
   * return 1. If the rescale_residual_by_volume is set to true, the method
   * returns the square root of the global volume of the triangulation.
   *
   * @return Rescale metric.
   */
  virtual double get_residual_rescale_metric() const = 0;

  /**
   * @brief Default way to evaluate the residual for the nonlinear solver, which
   * is simply the norm of the RHS vector. Some application may use more complex
   * evaluation of the residual and override this method.
   *
   * @return l2 norm of the RHS vector.
   */
  virtual double get_current_residual() {
...
PhysicsSolver<VectorType>::PhysicsSolver(
  ...
  case Parameters::NonLinearSolver::SolverType::newton:
    physics_solving_strategy = new NewtonNonLinearSolverStrategy<VectorType>(
...
template <typename VectorType>
void PhysicsSolver<VectorType>::solve_governing_system() {
