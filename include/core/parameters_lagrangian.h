...
enum RollingResistanceMethod : std::uint8_t { none, constant, viscous, epsd };

enum class SizeDistributionType : std::uint8_t { uniform, normal, custom };
...
public:
  ...
  static void declareDefaultEntry(ParameterHandler &prm);
  void parse_particle_properties(const unsigned int &particle_type,
...
template <int dim> class InsertionInfo {
public:
  // Insertion method
  enum class InsertionMethod { file, list, plane, volume } insertion_method;
...
template <int dim> class ModelParameters {
public:
  // Load balance method
 ...
 */
template <int dim> class ForceTorqueOnWall {
public:
...
template <int dim> class FloatingWalls {
public:
  ...
  void parse_parameters(ParameterHandler &prm);
  static void declareDefaultEntry(ParameterHandler &prm);
  void parse_floating_wall(ParameterHandler &prm);
...
public:
  ...
  void parse_parameters(ParameterHandler &prm);
  static void declareDefaultEntry(ParameterHandler &prm);
  void parse_boundary_conditions(const ParameterHandler &prm);
...
template <int dim> class GridMotion {
public:
  // Grid motion type
  enum class MotionType { translational, rotational, none } motion_type;
...
template <int dim> class FloatingGrid {
public:
 ...
 * ray tracing class.
 */
template <int dim> struct ParticleRayTracing {
