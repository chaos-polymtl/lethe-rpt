...
/**
 * @brief Define the behavior of a specific particle for the sharp interface IB
 * solver.
 *
 ...
 */
template <int dim> class IBParticle {
public:
  /**
   ...
   */
  void initialize_previous_solution();
   ...
   */
  std::vector<double> get_properties();

  /**
   * @brief Return the number of DEM properties, for visualization.
   */
  inline unsigned int get_number_properties() {
    return DEM::CFDDEMProperties::PropertiesIndex::n_properties;
  }

  /**
   * @brief Compute the evaluation of the signed distance function of this
   * shape.
   *
   ...
   */
  inline double get_levelset(const Point<dim> &p) { return shape->value(p); }
   ...
   * @brief See overloaded function.
   */
  void closest_surface_point(const Point<dim> &p, Point<dim> &closest_point);

  /**
   * @brief Determine if the given point is inside the crown for which the
   * limits are defined by inner and outer radius factors. An effective radius
   * is used for non spherical shapes.
   *
  ...
  bool is_inside_crown(
      const Point<dim> &evaluation_point, const double outer_radius,
      const double inside_radius, const bool absolute_distance,
      const typename DoFHandler<dim>::active_cell_iterator &cell_guess);

  /**
   * @brief See overloaded function.
   */
  bool is_inside_crown(const Point<dim> &evaluation_point,
                       const double outer_radius, const double inside_radius,
   ...
   */
  inline void set_position(const Point<dim> position) {
   ...
   */
  void initialize_shape(const std::string &type,
                        const std::vector<double> &shape_arguments);

  /**
   * @brief Set up a shape in accordance with the given type and raw text
   * argument.
   *
   ...
   * updated.
   */
  inline void set_position(const double position_component,
   ...
   */
  void set_orientation(const Tensor<1, 3> &orientation);
   ...
   */
  void set_layer_thickening(const double layer_thickening) {
  ...
  /**
   * @brief Update precalculations if needed, then compute and remove
   * superfluous data.
   *
   * @param[in,out] updated_dof_handler Reference to the new dof_handler.
   *
  ...
  /**
   * @brief Position, which corresponds to the centroid for most implemented
   * shapes.
   */
  Point<dim> position;

  /**
   * @brief Positions at the last n time steps. n depends on the time
   * integration scheme.
   */
  ...
  /**
   * @brief Last non-linear correction (without relaxation) to the angular
   * velocity.
   */
  ...
  /**
   * @brief Total angular impulsion felt by the particle during the current time
   * step.
   */
  ...
  /**
   * @brief Previous iteration's relaxation parameter used for the translational
   * velocity iteration.
   */
  double previous_local_alpha_velocity;

  /**
   * @brief Previous iteration's relaxation parameter used for the angular
   * velocity iteration.
   */
