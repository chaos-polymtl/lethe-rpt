...
/**
 * @brief Extract the maximum number of all variable size sections within the
 * parameter file
 *
...
enum class Verbosity : std::uint8_t { quiet, verbose, extra_verbose };
 ...
 */
enum class FluidIndicator : std::uint8_t { fluid0, fluid1, both };

/**
 * @brief SimulationControl - Defines the parameter that control the flow of the
 * simulation as well as the frequency of the output of the solutions.
 */
...
struct SimulationControl {
  ...
  enum class OutputControl : std::uint8_t { iteration, time } output_control;
...
/**
 * @brief Tanh-based physical properties model to handle properties in/out of
 * immersed solids
 */
...
/**
 * @brief MaterialInteractions - Class that defines physical properties due to
 * interactions between two different materials (either fluid-fluid or
 * fluid-solid).
 */
...
public:
   ...
   */
  void declare_parameters(ParameterHandler &prm, unsigned int id) const;
   ...
   * fundamental dimensions (length, time, mass, temperature) of the problem.
   */
  void parse_parameters(ParameterHandler &prm, const unsigned int id,
                        const Parameters::Dimensionality &dimensions);
};
 ...
 * "solid" domain.
 */
template <int dim> struct ConstrainSolidDomain {
   ...
   *
   */
  static void declare_default_entries(ParameterHandler &prm);
   ...
   */
  void parse_constraint_parameters(const ParameterHandler &prm,
                                   const unsigned int constraint_id);
};

/**
 * @brief Stabilization - Defines parameters for an advanced control over the
 * stabilization strategy used by the solvers.
 */
struct Stabilization {
  ...
  enum class ScalarLimiters : std::int8_t { moe, none } scalar_limiter;
...
/**
 * @brief Timer - Defines the parameters that control the timing of the
 * simulation. Lethe supports advanced timing features which supports the
 * monitoring of specific subsections of the software to evaluate the relative
 * workload.
 */
struct Timer {
  // Time measurement in the simulation. None, at each iteration, only at the
  // end
  enum class Type : std::int8_t { none, iteration, end };
...
struct NonLinearSolver {
  ...
  enum class SolverType : std::int8_t { newton, inexact_newton, kinsol_newton };
  ...
  void parse_parameters(ParameterHandler &prm, const std::string &physics_name);
};
...
struct LinearSolver {
  ...
  enum class SolverType : std::int8_t { gmres, bicgstab, direct };
  ...
  enum class PreconditionerType : std::int8_t { ilu, amg, lsmg, gcmg };
  ...
  /// Type of multigrid
  enum class MultigridCoarseningSequenceType : std::int8_t { h, p, hp, ph };
  ...
  enum class CoarseGridSolverType : std::int8_t { gmres, amg, ilu, direct };
...
struct Mesh {
  // GMSH or dealii
  enum class Type : std::int8_t { gmsh, dealii, periodic_hills, cylinder };
  Type type;
...
/**
 * @brief Mesh Adaptation Parameters that can differ for each refinement
 * variable
 */
...
struct MeshAdaptation {
  ...
  enum class Type : std::int8_t { none, uniform, kelly } type;
  ...
  enum class FractionType : std::int8_t { number, fraction } fractionType;
...
struct VelocitySource {
  ...
  enum class RotatingFrameType : std::int8_t { none, srf };
...
template <int dim> class IBParticles {
public:
  void declare_parameters(ParameterHandler &prm);
  void declare_default_entry(ParameterHandler &prm, unsigned int index);
  void parse_parameters(ParameterHandler &prm);
  // Vector of particles
 ...
 */
template <int dim> struct Mortar {
