...
template <int dim, int spacedim = dim> class SolidBase {
public:
  // Member functions
  SolidBase(
   ...
   */
  void setup_triangulation(const bool restart);

  /**
   * @brief Sets-up particles with particle handler, in the fluid triangulation
   * domain that holds the particles of the solid according to a specific
   * quadrature
   */
  void setup_displacement();

  /**
   * @brief Creates a particle handler in the fluid triangulation domain
   */
  void setup_particles_handler();
   ...
   */
  void load_triangulation(const std::string &filename_tria);

  /**
   * @brief Loads a particle handler in the fluid triangulation domain that
   * holds the particles of the solid according to a specific quadrature, and
   * sets up dofs
   */
  void load_particles(const std::string &filename_part);

  /**
   * @return the reference to the std::shared_ptr of a
   * Particles::ParticleHandler<spacedim> that contains the solid particle
   * handler
   */
   ...
   */
  DoFHandler<dim, spacedim> &get_solid_dof_handler();

  /**
   * @return the reference to the displacement dof handler
   */
  DoFHandler<dim, spacedim> &get_displacement_dof_handler();

  /**
   * @return the reference to the displacement vector
   */
  GlobalVectorType &get_displacement_vector();

  /**
   * @return Function<spacedim> of the solid velocity
   */
  Function<spacedim> *get_solid_velocity();

  Function<spacedim> *get_solid_temperature();

  /**
   * @brief Updates particle positions in solid_particle_handler by integrating
   * velocity using an explicit Runge-Kutta 4 method.
   *
   * @param time_step The time_step value for this iteration
   *
   * @param initial_time The initial time (time t) of the timestep. This is used
   * to set the time of the velocity function.
   */
  void integrate_velocity(double time_step, double initial_time);
   ...
   */
  void move_solid_triangulation(double time_step, double initial_time);

  /**
   * @brief Moves the dofs of the solid_triangulation by using the displacement
   * vector. This is only use to move the solid triangulation at the correct
   * location when the simulation is restarted.
   */
  void move_solid_triangulation_with_displacement();
   ...
   *
   */
  void rotate_grid(const double angle, const Tensor<1, 3> &axis);
   ...
   */
  void translate_grid(const Tensor<1, 3> &translate);

  /**
   * @brief Updates the time in the function used to describe the solid
   * temperature
   */
  void update_temperature_time(double time);

  /**
   * @brief read solid base triangulation checkpoint
   */
  void read_checkpoint(const std::string &prefix_name);

  /**
   * @brief write solid base triangulation checkpoint
   */
  void write_checkpoint(const std::string &prefix_name);
