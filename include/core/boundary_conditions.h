...
DeclException1(
    EmissivityError, double,
    << "Emissivity : " << arg1
    << " cannot be larger than 1.0 (black body) or smaller than 0.0");

DeclException1(NavierStokesBoundaryDuplicated, types::boundary_id,
               << "Fluid dynamics boundary id: " << arg1
               << " has already been declared as a boundary condition");

DeclException1(HeatTransferBoundaryDuplicated, types::boundary_id,
               << "Heat transfer boundary id: " << arg1
               << " has already been declared as a boundary condition");

DeclException1(TracerBoundaryDuplicated, types::boundary_id,
               << "Tracer boundary id: " << arg1
               << " has already been declared as a boundary condition");

DeclException1(CahnHilliardBoundaryDuplicated, types::boundary_id,
               << "Cahn Hilliard boundary id: " << arg1
               << " has already been declared as a boundary condition");

DeclException1(VOFBoundaryDuplicated, types::boundary_id,
...
/**
 * @brief This class manages the functions associated with function-type
 * boundary conditions of the Navier-Stokes equations
 *
 */
template <int dim> class NSBoundaryFunctions {
public:
  // Velocity components
...
/**
 * @brief This class manages the functions associated with function-type
 * boundary conditions of the Cahn-Hilliard equations
 *
 */
template <int dim> class CahnHilliardBoundaryFunctions {
public:
 ...
 * coherently.
 */
template <int dim> class NSBoundaryConditions : public BoundaryConditions {
public:
  ...
  void parse_boundary(ParameterHandler &prm);
  void declare_default_entry(ParameterHandler &prm,
   ...
   * @param number_of_boundary_conditions The number of boundary conditions to
   * be declared. This parameter is generally pre-parsed from a first read of
   * the prm file.
   */
 ...
 * Used in tests only
 */
template <int dim> void NSBoundaryConditions<dim>::createNoSlip() {
 ...
 *
 * @param prm A parameter handler which is currently used to parse the
 * simulation information
 *
 * @param default_boundary_id Default value of the boundary id. This corresponds
 * to the number of the boundary condition subsection.
 */
template <int dim>
void NSBoundaryConditions<dim>::declare_default_entry(
    ParameterHandler &prm, const types::boundary_id default_boundary_id) {
  prm.declare_entry(
      "type", "none",
      Patterns::Selection("none|noslip|slip|function|periodic|pressure|"
                          "function weak|partial slip|outlet"),
      "Type of boundary condition"
      "Choices are <noslip|slip|function|periodic|pressure|function "
      "weak|partial slip|outlet>.");

  prm.declare_entry("id", Utilities::to_string(default_boundary_id, 2),
                    Patterns::List(Patterns::Integer()),
                    "Mesh id for boundary conditions. Default entry is -1 to "
                    "ensure that the id is set by the user");

  prm.declare_entry("periodic id", "-1", Patterns::Integer(),
                    "Mesh id for periodic face matching. Default entry is -1 "
                    "to ensure that the periodic id is set by the user");

  prm.declare_alias("periodic id", "periodic_id", true);

  prm.declare_entry("periodic direction", "0", Patterns::Integer(),
  ...
  prm.declare_entry("beta", "1", Patterns::Double(),
                    "Penalty parameter for weak boundary condition imposed "
                    "through Nitsche's method or outlets");

  prm.declare_entry(
      "boundary layer thickness", "0", Patterns::Double(),
      "Thickness of the boundary layer used to calculate the penalty parameter "
      "for partial slip boundary condition in tangent direction imposed "
      "through Nitsche's method or outlets");
}
...
void NSBoundaryConditions<dim>::parse_boundary(ParameterHandler &prm) {
  ...
  AssertThrow(boundary_ids.size() > 0,
              ExcMessage("A boundary id has not been set for one of the fluid "
                         "dynamics boundary conditions. Please ensure that the "
                         "id is set for every boundary condition."));
  ...
  for (const auto boundary_id : boundary_ids) {
    ...
    if (op == "periodic") {
      types::boundary_id periodic_boundary_id = prm.get_integer("periodic id");
...
template <int dim>
void NSBoundaryConditions<dim>::declare_parameters(
    ParameterHandler &prm, const unsigned int number_of_boundary_conditions) {
  prm.enter_subsection("boundary conditions");
  {
    prm.declare_entry("number",
                      Utilities::int_to_string(number_of_boundary_conditions),
                      Patterns::Integer(), "Number of boundary conditions");
    prm.declare_entry(
        "time dependent", "false", Patterns::Bool(),
        "Bool to define if the boundary condition is time dependent");

    prm.declare_entry(
        "fix pressure constant", "false", Patterns::Bool(),
...
template <int dim> class HTBoundaryConditions : public BoundaryConditions {
public:
 ...
 */
template <int dim>
void HTBoundaryConditions<dim>::declare_default_entry(
    ParameterHandler &prm, const types::boundary_id default_boundary_id) {
  prm.declare_entry(
      "type", "noflux",
      Patterns::Selection(
 ...
 */
template <int dim>
void HTBoundaryConditions<dim>::declare_parameters(
    ParameterHandler &prm, const unsigned int number_of_boundary_conditions) {
  prm.enter_subsection("boundary conditions heat transfer");
  {
    ...
    prm.declare_entry("Stefan-Boltzmann constant", "0.000000056703",
...
template <int dim>
void HTBoundaryConditions<dim>::parse_boundary(ParameterHandler &prm) {
  // Parse the list of boundary ids
  std::vector<types::boundary_id> boundary_ids =
      convert_string_to_vector<types::boundary_id>(prm, "id");
  ...
  // Check if the list contains at least one boundary id
  AssertThrow(boundary_ids.size() > 0,
              ExcMessage("A boundary id has not been set for one of the heat "
                         "transfer boundary conditions. Please ensure that the "
                         "id is set for every boundary condition."));
...
template <int dim>
void HTBoundaryConditions<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("boundary conditions heat transfer");
  {
    this->number_of_boundary_conditions = prm.get_integer("number");
    this->time_dependent = prm.get_bool("time dependent");
...
template <int dim> class TracerBoundaryConditions : public BoundaryConditions {
public:
  std::map<types::boundary_id, std::shared_ptr<Functions::ParsedFunction<dim>>>
      tracer;
 ...
 */
template <int dim>
void TracerBoundaryConditions<dim>::declare_default_entry(
    ParameterHandler &prm, const types::boundary_id default_boundary_id) {
  prm.declare_entry("type", "outlet",
                    Patterns::Selection("dirichlet|outlet|periodic"),
                    "Type of boundary condition for tracer"
                    "Choices are <dirichlet|outlet|periodic>.");

  prm.declare_entry("id", Utilities::int_to_string(default_boundary_id, 2),
                    Patterns::Integer(), "Mesh id for boundary conditions");
 ...
 */
template <int dim>
void TracerBoundaryConditions<dim>::declare_parameters(
    ParameterHandler &prm, const unsigned int number_of_boundary_conditions) {
...
template <int dim>
void TracerBoundaryConditions<dim>::parse_boundary(ParameterHandler &prm) {
  // Parse the list of boundary ids
  std::vector<types::boundary_id> boundary_ids =
      convert_string_to_vector<types::boundary_id>(prm, "id");
  ...
  // Check if the list contains at least one boundary id
  AssertThrow(boundary_ids.size() > 0,
              ExcMessage("A boundary id has not been set for one of the tracer "
                         "boundary conditions. Please ensure that the id is "
                         "set for every boundary condition."));
...
template <int dim>
void TracerBoundaryConditions<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("boundary conditions tracer");
  {
    this->number_of_boundary_conditions = prm.get_integer("number");
    this->time_dependent = prm.get_bool("time dependent");
...
/**
 * @brief This class manages the boundary conditions for the Cahn-Hilliard
 * solver It introduces the boundary functions and declares the boundary
...
/**
 * @brief Declares the default parameters for the Cahn-Hilliard boundary
 * conditions
 *
 * @param prm A parameter handler which is currently used to parse the
 * simulation information.
 ...
 */
template <int dim>
void CahnHilliardBoundaryConditions<dim>::declare_default_entry(
    ParameterHandler &prm, const types::boundary_id default_boundary_id) {
  prm.declare_entry(
      "type", "none",
      Patterns::Selection(
  ...
  prm.declare_entry("angle value", "0", Patterns::Double(),
                    "Inner angle of contact between the fluid 1 and the "
                    "boundary (in degrees)");
 ...
 */
template <int dim>
void CahnHilliardBoundaryConditions<dim>::declare_parameters(
    ParameterHandler &prm, const unsigned int number_of_boundary_conditions) {
...
template <int dim>
void CahnHilliardBoundaryConditions<dim>::parse_boundary(
  ...
  // Check if the list contains at least one boundary id
  AssertThrow(boundary_ids.size() > 0,
              ExcMessage("A boundary id has not been set for one of the Cahn "
                         "Hilliard boundary conditions. Please ensure that the "
                         "id is set for every boundary condition."));
  // Loop through all boundary ids to ensure that they are all non-negative
  // and unique
  for (const auto boundary_id : boundary_ids) {
...
template <int dim>
void CahnHilliardBoundaryConditions<dim>::parse_parameters(
    ParameterHandler &prm) {
  prm.enter_subsection("boundary conditions cahn hilliard");
  {
    this->number_of_boundary_conditions = prm.get_integer("number");
...
template <int dim> class VOFBoundaryConditions : public BoundaryConditions {
public:
  std::map<types::boundary_id, std::shared_ptr<Functions::ParsedFunction<dim>>>
      phase_fraction;
 ...
 */
template <int dim>
void VOFBoundaryConditions<dim>::declare_default_entry(
    ParameterHandler &prm, const types::boundary_id default_boundary_id) {
 ...
 */
template <int dim>
void VOFBoundaryConditions<dim>::declare_parameters(
    ParameterHandler &prm, const unsigned int number_of_boundary_conditions) {
...
template <int dim>
void VOFBoundaryConditions<dim>::parse_boundary(ParameterHandler &prm) {
  // Parse the list of boundary ids
  std::vector<types::boundary_id> boundary_ids =
      convert_string_to_vector<types::boundary_id>(prm, "id");
  ...
  // Check if the list contains at least one boundary id
  AssertThrow(boundary_ids.size() > 0,
              ExcMessage("A boundary id has not been set for one of the VOF "
                         "boundary conditions. Please ensure that the id is "
                         "set for every boundary condition."));
...
template <int dim>
void VOFBoundaryConditions<dim>::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("boundary conditions VOF");
  {
    this->number_of_boundary_conditions = prm.get_integer("number");
    this->time_dependent = prm.get_bool("time dependent");
...
/**
 * @brief This class implements a boundary conditions for the Navier-Stokes
 * equation where the velocity component are defined using individual functions
 */
template <int dim> class NavierStokesFunctionDefined : public Function<dim> {
  Functions::ParsedFunction<dim> *u;
  Functions::ParsedFunction<dim> *v;
  Functions::ParsedFunction<dim> *w;
 ...
 *
 * @param component The vector component of the boundary condition (0-x, 1-y and
 * 2-z)
 */
template <int dim>
double
...
/**
 * @brief This class implements a pressure boundary condition for the
...
public:
  NavierStokesPressureFunctionDefined(Functions::ParsedFunction<dim> *p_p)
      : Function<dim>(dim + 1), p(p_p) {}

  double value(const Point<dim> &point, unsigned int component) const override;
};
 ...
 */
template <int dim>
double NavierStokesPressureFunctionDefined<dim>::value(
...
/**
 * @brief Class that implements a boundary conditions for the Cahn-Hilliard
 * equation where the phase and chemical potential are defined using individual
 * functions
 */
template <int dim> class CahnHilliardFunctionDefined : public Function<dim> {
private:
  Functions::ParsedFunction<dim> *phi;

public:
  CahnHilliardFunctionDefined(Functions::ParsedFunction<dim> *p_phi)
      : Function<dim>(2), phi(p_phi) {}
...
/**
 * @brief Calculates the value of a Function-type for the Cahn-Hilliard
 * equations
 *
