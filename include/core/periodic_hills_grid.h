...
/**
 * @brief Class that creates a hyper_rectangle and transforms it to obtain the
 * hill geometry with the hill_geometry function. It also attaches a manifold to
 * the geometry.
 */

template <int dim, int spacedim> class PeriodicHillsGrid {
public:
  PeriodicHillsGrid(const std::string &grid_arguments);
  Point<spacedim> static hill_geometry(const Point<spacedim> &p,
                                       double spacing_y, double alpha);
...
/**
 * @brief The push_forward & the pull_back classes create the vector_value
 * functions gradient function and because it inherits from Function<spacedim>.
 * (formula is currently Euler and can be changed. See AutoDerivativeFunction
 * documentation)
 */
...
public:
  PeriodicHillsPushForward(double spacing_y, double alpha)
      : AutoDerivativeFunction<spacedim>(1e-6, spacedim), spacing_y(spacing_y),
        alpha(alpha) {}
...
public:
  ...
  virtual void vector_value(const Point<spacedim> &np,
                            Vector<double> &values) const override;

  virtual double value(const Point<spacedim> &np,
                       const unsigned int component) const override;
...
/**
 * @brief vector_value. This function is used to construct the geometry
 * manifold. It changes the original point (op) of the transformed
 * hyper_rectangle with hill_geometry to a new point (np) of the hill grid with
 * per_hills_grid function.
 *
 * @param p. A point in space
 *
 * @param values. The vector of values which will be calculated at the position
 * p.
 */
template <int dim, int spacedim>
void PeriodicHillsPushForward<dim, spacedim>::vector_value(
    const Point<spacedim> &op, Vector<double> &values) const {
...
/**
 * @brief value. The value function does the same thing than vector_value for
 * one component. This implementation is needed to use the gradient function
 * inherited by AutoDerivativeFunction.
 *
 ...
 */
template <int dim, int spacedim>
double PeriodicHillsPushForward<dim, spacedim>::value(
    const Point<spacedim> &op, const unsigned int component) const {
  const Point<spacedim> np =
      PeriodicHillsGrid<dim, spacedim>::hill_geometry(op, spacing_y, alpha);
  return np[component];
}
 ...
 */
template <int dim, int spacedim>
void PeriodicHillsPullBack<dim, spacedim>::vector_value(
    const Point<spacedim> &np, Vector<double> &values) const {
  double x = np[0];
  double max_y = 3.035;
  double max_x = 9.0;
 ...
 */
template <int dim, int spacedim>
double PeriodicHillsPullBack<dim, spacedim>::value(
    const Point<spacedim> &np, const unsigned int component) const {
  double x = np[0];
  double max_y = 3.035;
  double max_x = 9.0;
  double flat_region_length = 5.142;
...
/**
 * @brief The hill_geometry function calculates all the domain of the geometry
 * with 6 polynomials depending the x position. (See Hill Geometry Definition
 * file : https://turbmodels.larc.nasa.gov/Other_LES_Data/2dhill_periodic.html)
 ...
 */
template <int dim, int spacedim>
Point<spacedim> PeriodicHillsGrid<dim, spacedim>::hill_geometry(
    const Point<spacedim> &p, double spacing_y, double alpha) {
  ...
  if (x >= 0 && x < 9) {
    ...
    y += pos_y *
         (a1 + b1 * new_x + c1 * std::pow(new_x, 2) + d1 * std::pow(new_x, 3));
    y_0 = (a1 + b1 * x + c1 * std::pow(x, 2) + d1 * std::pow(x, 3));
    diff = y_0 - 28.0;
    if (diff > 0)
      y -= pos_y * diff;
    ...
    y += pos_y *
         (a6 + b6 * new_x + c6 * std::pow(new_x, 2) + d6 * std::pow(new_x, 3));
    if (y < 0)
      y = 0;
  } else
    y += 0;
...
/**
 * @brief make_grid. The make_grid function generates a hyper rectangle of the
 * size of the domain and then transforms it to the hill geometry. It also
 * constructs the geometry manifold with FunctionManifold and finally sets the
 * manifold.
 *
 * @param triangulation. The triangulation object on which the grid is generated
 */
template <int dim, int spacedim>
void PeriodicHillsGrid<dim, spacedim>::make_grid(
  ...
  if (dim == 2) {
    if (alpha != 1 && (repetitions_x > 1 || repetitions_y > 1))
      throw std::logic_error("When alpha parameter is not 1, repetition "
                             "parameters should be all set to 1");

    GridGenerator::subdivided_hyper_rectangle(triangulation, repetitions,
                                              Point<dim>(0.0, 0.0),
                                              Point<dim>(9.0, 3.035), true);
  } else if (dim == 3) {
    if (alpha != 1 &&
    ...
    GridGenerator::subdivided_hyper_rectangle(
        triangulation, repetitions, Point<dim>(0.0, 0.0, 0.0),
        Point<dim>(9.0, 3.035, 4.5), true);
  }
  ...
  static const FunctionManifold<dim, spacedim, spacedim> manifold_func(
      std::make_unique<PeriodicHillsPushForward<dim, spacedim>>(spacing_y,
                                                                alpha),
