...
/**
 * @brief Non-linear solver for non-linear systems of equations which uses an
 * inexact Newton method with \f$\alpha\f$ relaxation to ensure that the
 * residual is monotonically decreasing. It allows to reuse the matrix and avoid
 * its reassembly for every Newton step.
 *
 */
...
template <typename VectorType>
void InexactNewtonNonLinearSolverStrategy<VectorType>::solve() {
  double global_res;
  double current_res;
  double last_res;
  ...
  while ((global_res > this->params.tolerance) &&
    ...
    for (double alpha = 1.0; alpha > 1e-1; alpha *= 0.5) {
      ...
      if (this->params.verbosity != Parameters::Verbosity::quiet) {
        solver->pcout << "\talpha = " << std::setw(6) << alpha << std::setw(0)
                      << " res = "
                      << std::setprecision(this->params.display_precision)
                      << std::setw(6) << current_res;

        solver->output_newton_update_norms(this->params.display_precision);
      }
      ...
      if (alpha_iter != 0 and current_res > last_alpha_res) {
        ...
        if (this->params.verbosity != Parameters::Verbosity::quiet) {
          solver->pcout << "\t\talpha value was kept at alpha = " << alpha
                        << " since alpha = " << alpha / 2
  ...
  if ((global_res > this->params.tolerance) &&
    ...
    throw(std::runtime_error("Stopping simulation because the non-linear "
                             "solver has failed to converge"));
  }
}
